# Plan ID 1 — Raise Go Coverage > 80% by Extracting `cmd/` Logic and Introducing Testable Interfaces (Working Copy for Continuation)

Active path: 1 → 4 → 4.x → 1.5 → 1.5.cont
Parent: —

This file is a working copy of the master plan (2025-11-09-2205-raise-coverage-refactor.md), updated to mark completed items and to continue execution per `.junie/guidelines.md`. It keeps the original structure and acceptance criteria, with statuses reconciled based on recent progress.

Notes:
- Branching: never commit to main; use feature branches; Conventional Commits.
- TDD: table‑driven tests, per‑test assert helpers, no ifs in test bodies; mocks via mockery.
- No network/real DB in unit tests; use interfaces and mocks; deterministic fixtures.
- Do not modify `src/`.
...
### 4.x Other commands
- Commands tracked here: `etl-importer`, `weather-import`, `weather-worker`, `team-predictor`, `team-select`, `evaluate`, `tools/migrate` — repeat the pattern introducing needed interfaces.

Per-command progress:
- etl-importer: ✓ complete
- weather-import: ✓ complete
- weather-worker: ✓ complete
- team-predictor: ✓ complete
- team-select: ✓ complete (see plan 1.5.4 and reconciliation plan)
- evaluate: * in progress (pending extraction)
- tools/migrate: * in progress (pending thinning)
...
- PR 3..N: Extract remaining commands (1–2 per PR) with tests. * (evaluate, tools/migrate remain)
...
- 1 (root)
  ...
  - 4 (Per-command extractions)
    - 4.1 export-dataset — Status: ✓
    - 4.2 cricsheet-importer — Status: ✓
    - 4.3 backfill-fielding — Status: ✓
    - 4.x remaining commands — Status: * (evaluate, tools/migrate pending)
  - 5 (Logging)
  - 6 (Adapters)
  - 7 (Testing tooling)
  - 8 (CI)
...
## Next Immediate Actions (per .junie/guidelines.md)
- Start 1.5.cont.1 — Extract `evaluate` on branch `feat/evaluate-extract` (TDD cadence):
  - Create `internal/cli/evaluate`, `internal/commands/evaluate`, `internal/services/evaluate` with interfaces and mockery tags.
  - Thin `cmd/evaluate`.
  - Add table-driven tests to reach package thresholds.
  - Verify with: `cd go-app && make mocks && make test && make coverage && make coverage-func`.
- Then 1.5.cont.2 — Thin `tools/migrate` on branch `feat/migrate-runner` with similar pattern and tests.
