...
describe("MatchCompareDbTab", () => {
  ...
  it("happy path: compares predictions and shows correct/incorrect outcome", async () => {
    ...
    fireEvent.change(
      screen.getAllByLabelText("match-id-input")[0] as HTMLInputElement,
      { target: { value: "99" } },
    );
    fireEvent.change(
      screen.getAllByLabelText("asof-input")[0] as HTMLInputElement,
    ...
    expect(
      within(predictedRow as HTMLElement).getByText("Alpha"),
    ).toBeInTheDocument();
    const actualRow = screen.getByText(/Actual winner/i);
    expect(
      within(actualRow as HTMLElement).getByText("Alpha"),
    ).toBeInTheDocument();
    expect(screen.getByText(/Correct/i)).toBeInTheDocument();
  });

  it("shows friendly error on 404 not found", async () => {
    (api.getMatchSquads as any).mockRejectedValue(
      new Error('HTTP 404 Not Found: {"code":"NOT_FOUND"}'),
    );
