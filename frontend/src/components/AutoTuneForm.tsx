...
const AutoTuneForm: React.FC<AutoTuneFormProps> = ({
  model, onModelChange, format, onFormatChange,
  rescreen, onRescreenChange, cutoff, onCutoffChange,
  ...
  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mb: 1.5 }}>
    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
      <FormControl size="small" sx={{ minWidth: 140 }}>
        ...
        <Select labelId="autotune-model-label" value={model} label="Model" onChange={(e) => onModelChange(e.target.value)}>
          {AUTO_TUNE_MODELS.map((o) => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)}
        </Select>
      </FormControl>
      <FormControl size="small" sx={{ minWidth: 140 }}>
        <InputLabel id="autotune-format-label">Format</InputLabel>
        <Select labelId="autotune-format-label" value={format} label="Format" onChange={(e) => onFormatChange(e.target.value)}>
          {AUTO_TUNE_FORMATS.map((o) => <MenuItem key={o.value || 'all'} value={o.value}>{o.label}</MenuItem>)}
        </Select>
      </FormControl>
    </Box>
    <FormControlLabel
      control={<Checkbox checked={rescreen} onChange={(e) => onRescreenChange(e.target.checked)} size="small" />}
      label="Rescreen: start from scratch (full algorithm search, ignore prior tuning)"
    />
    <TextField
      size="small" label="Cutoff (optional, RFC3339)" placeholder="2024-01-01T00:00:00Z"
      value={cutoff} onChange={(e) => onCutoffChange(e.target.value)}
      sx={{ maxWidth: 320 }} helperText="Training data cutoff for API. Leave empty to use default."
    />
    <Box>
