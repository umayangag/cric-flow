...
const MatchCompareDbTab: React.FC = () => {
  ...
  const onCompare = async () => {
    ...
    try {
      ...
      // Determine winners
      const predictedWinnerIndex = probs[0] === probs[1] ? null : (probs[0] > probs[1] ? 0 : 1);
      const actualWinnerIndex = data.squads[0].actual_win === 1 ? 0 : data.squads[1].actual_win === 1 ? 1 : null;
      ...
      if (msg.includes('HTTP 404')) {
        ...
        setError('Incomplete squads for this match (one or both teams incomplete).');
      } else {
        setError(msg);
      }
    } finally {
  ...
  return (
    <div>
      <p style={{ marginTop: 0 }}>
        Compare ML prediction vs actual result for a specific match from DB squads. Enter a match ID, select an as-of
        date and optional format, then click Compare.
      </p>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
        ...
        <label>
          As of:{' '}
          <input type="date" value={asof} onChange={(e) => setAsof(e.target.value)} aria-label="asof-input" />
        </label>
        <label>
          Format:{' '}
          <select value={format} onChange={(e) => setFormat(e.target.value)} aria-label="format-select">
            {formats.map((f) => (
        ...
        <button onClick={onCompare} disabled={!canCompare} aria-label="compare-button">
      ...
      {!result ? (
        ...
        <div>
          ...
          <div>
            <strong>{result.teams[0]}</strong>: {(result.probs[0] * 100).toFixed(2)}% &nbsp; | &nbsp;
            <strong>{result.teams[1]}</strong>: {(result.probs[1] * 100).toFixed(2)}%
          </div>
          ...
          {result.predictedWinnerIndex != null && result.actualWinnerIndex != null && (
            <div style={{ marginTop: 8 }}>
              Outcome:{' '}
