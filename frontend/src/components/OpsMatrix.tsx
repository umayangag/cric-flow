...
export const OpsMatrix: React.FC<Props> = ({ type, title, data }) => {
    ...
    <div style={{ display: 'grid', gap: 8 }}>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {FORMATS.map((f) => {
          ...
          const color = st === 'ok' ? '#17431d' : st === 'stale' ? '#54450f' : st === 'missing' ? '#4a1010' : '#333';
          const txt = st ?? 'unknown';
          return (
            <div key={f} style={cellStyle(color)} title={`status: ${txt}`} data-testid={`precompute-${f}`}>
              <strong>{f}</strong>
    ...
    <div style={{ display: 'grid', gap: 8 }}>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {FORMATS.map((f) => {
          ...
          const titleStr = Array.isArray(files) ? files.map((x: any) => x?.name).filter(Boolean).join(', ') : '';
          return (
  ...
  const subCell = (ok: boolean, loaded?: boolean) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {loaded ? <span title="loaded" style={{ width: 7, height: 7, background: '#48d597', borderRadius: 999 }} /> : null}
    </div>
  );
          ...
          return (
            <div key={f} style={cellStyle(color)} data-testid={`artifacts-${f}`}>
              ...
              <div style={{ display: 'grid', gap: 6, marginTop: 6 }}>
                <div>{subCell(bat?.exists === true, bat?.loaded === true)} <small>batting</small></div>
                <div>{subCell(bowl?.exists === true, bowl?.loaded === true)} <small>bowling</small></div>
              </div>
            </div>
          );
        })}
