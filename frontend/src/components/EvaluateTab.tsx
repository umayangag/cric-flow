...
const EvaluateTab: React.FC = () => {
  ...
  const runEvaluation = async () => {
    try {
      ...
      if (nextSeason == null)
        throw new Error('Could not determine immediate next season after cutoff.');
  ...
  return (
    <div>
      <p style={{ marginTop: 0 }}>
        Upload a CSV with player-level rows and select a cutoff date X. The app evaluates the
