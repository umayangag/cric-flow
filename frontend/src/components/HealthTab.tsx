      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <Paper elevation={1} sx={{ p: 2 }}>
            ...
            <KeyValueList
              items={[
                { label: 'Latency', value: latencyApiMs != null ? `${latencyApiMs} ms` : '—' },
        ...
        <Grid item xs={12} md={6}>
          <Paper elevation={1} sx={{ p: 2 }}>
            ...
            <KeyValueList
              items={[
                { label: 'Latency', value: latencyMlMs != null ? `${latencyMlMs} ms` : '—' },
                { label: 'Last checked', value: lastCheckedLocal || '—' },
                { label: 'Loaded batting', value: mlData?.loaded_batting_formats?.join(', ') || '—' },
                { label: 'Loaded bowling', value: mlData?.loaded_bowling_formats?.join(', ') || '—' },
                { label: 'Models dir', value: mlData?.models_dir || '—' },
                { label: 'Legacy batting', value: mlData ? (mlData.legacy_batting_available ? 'Yes' : 'No') : '—' },
                { label: 'Legacy bowling', value: mlData ? (mlData.legacy_bowling_available ? 'Yes' : 'No') : '—' },
                (() => {
                  ...
                  return { label: 'Artifacts', value: `batting: ${batCount}, bowling: ${bowlCount}` };
                })(),
                (() => {
                  const all = [...(mlData?.artifacts?.batting || []), ...(mlData?.artifacts?.bowling || [])] as any[];
                  const total = all.reduce((acc, it) => acc + (typeof it?.size_bytes === 'number' ? it.size_bytes : 0), 0);
                  return { label: 'Total size', value: total > 0 ? formatBytes(total) : '—' };
                })(),
                (() => {
                  const all = [...(mlData?.artifacts?.batting || []), ...(mlData?.artifacts?.bowling || [])] as any[];
                  const latest = all
                  ...
                  return { label: 'Latest modified', value: isNaN(d.getTime()) ? '—' : d.toLocaleString() };
                })(),
              ]}
            />
          </Paper>
