...
export const AccuracyTrendChart: React.FC<AccuracyTrendChartProps> = ({
  ...
  if (!data || data.length === 0) {
    return (
      <div style={{ padding: 8, color: "#666" }}>
        No progressive data to display.
      </div>
    );
  }
  ...
  const x = (n: number) =>
    padding.left + ((n - 1) / Math.max(1, maxN - 1)) * innerW;
  const yLeft = (v: number) =>
    padding.top + (1 - v / Math.max(1e-9, maxLeft)) * innerH;
  ...
  const makePath = (key: string, useRightAxis = false) => {
    const pts = data
      ...
      .map(
        (d) =>
          [x(d.n), useRightAxis ? yRight(d[key]) : yLeft(d[key])] as [
            number,
            number,
          ],
      );
  ...
  const gridLines = Array.from(
    { length: gridY + 1 },
    (_, i) => padding.top + (i / gridY) * innerH,
  );
  ...
  return (
    <svg
      ...
      height={height}
      role="img"
      aria-label="Accuracy trend progressive averages chart"
    >
      {/* background */}
      ...
      {/* axes labels */}
      <text x={padding.left} y={padding.top - 6} fill="#333" fontSize={12}>
        MAE (left)
      </text>
      <text
      ...
      {leftValues.length > 0 && (
        <path
          d={makePath("player_runs_mae_avg")}
          fill="none"
          stroke="#1f77b4"
          strokeWidth={2}
        />
      ...
      {/* legend */}
      <g
        transform={`translate(${padding.left}, ${height - padding.bottom + 6})`}
      >
        ...
        <text x={170} y={4} fontSize={12}>
          team_runs_mae_avg
        </text>
        <circle cx={300} cy={0} r={4} fill="#d62728" />
        <text x={310} y={4} fontSize={12}>
          team_winner_accuracy_avg
        </text>
      </g>
    </svg>
  );
