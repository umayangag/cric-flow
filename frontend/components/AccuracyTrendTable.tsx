...
export const AccuracyTrendTable: React.FC<AccuracyTrendTableProps> = ({ rows }) => {
  const hasPlayerMAE = rows.some(r => typeof r.metrics?.player_runs_mae === 'number');
  const hasTeamMAE = rows.some(r => typeof r.metrics?.team_runs_mae === 'number');
  const hasWinnerAcc = rows.some(r => typeof r.metrics?.team_winner_accuracy === 'number');
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.match_id}>
              ...
              {hasPlayerMAE && <td style={tdRight}>{fmt(r.metrics?.player_runs_mae)}</td>}
              {hasTeamMAE && <td style={tdRight}>{fmt(r.metrics?.team_runs_mae)}</td>}
              {hasWinnerAcc && <td style={tdRight}>{fmt(r.metrics?.team_winner_accuracy, 0)}</td>}
            </tr>
          ))}
        </tbody>
      </table>
