...
function buildAccuracyTrendUrl(
  ...
  const envBase = (typeof process !== 'undefined' && (process as any).env && (process as any).env.NEXT_PUBLIC_API_BASE)
...
export async function fetchAccuracyTrend(
  ...
  if (!res.ok) {
    ...
    throw new Error(`accuracy-trend request failed: HTTP ${res.status} ${res.statusText}${body ? ` — ${body}` : ''}`);
  }
