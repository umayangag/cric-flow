...
const Page: React.FC = () => {
  ...
  const onLimit = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = Math.max(
      1,
      Math.min(500, parseInt(e.target.value || "0", 10) || 0),
    );
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <p style={{ color: "#555" }}>
        Visualize how prediction accuracy changes as we move from earlier to
        more recent matches. Use the filters below to select a format, date
        range, and teams. Team aggregates may use cached predictions depending
        on the cache mode.
      </p>
      ...
      <section style={panel}>
        ...
        <div style={row}>
          ...
          <select
            name="cache"
            value={(filters.cache as string) || "readwrite"}
            onChange={onInput}
          >
