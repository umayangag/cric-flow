...
import { AccuracyTrendResponse, fetchAccuracyTrend, AccuracyTrendFilters } from '../../lib/api';
...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    ...
    router.replace({ pathname: router.pathname, query: qs }, undefined, {
      shallow: true,
    });
  }, [filters, router]);
  ...
  const onMetricsToggle = (token: 'player' | 'team') => {
    setFilters((prev) => {
      const current = String(prev.metrics || '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
  ...
  const metricsState = useMemo(() => {
    const current = String(filters.metrics || '')
      .split(',')
      .map((s) => s.trim());
    return {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <p style={{ color: '#555' }}>
        Visualize how prediction accuracy changes as we move from earlier to more recent matches.
        Use the filters below to select a format, date range, and teams. Team aggregates may use
        cached predictions depending on the cache mode.
      </p>
      ...
      <section style={panel}>
        ...
        <div style={{ ...row, alignItems: 'center' }}>
          ...
          <label style={{ marginRight: 12 }}>
            <input
              type="checkbox"
              checked={metricsState.player}
              onChange={() => onMetricsToggle('player')}
            />{' '}
            player
          </label>
          <label>
            <input
              type="checkbox"
              checked={metricsState.team}
              onChange={() => onMetricsToggle('team')}
            />{' '}
            team
          </label>
        </div>
