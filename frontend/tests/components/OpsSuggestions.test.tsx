...
describe('OpsSuggestions', () => {
  ...
  it('copies commands when clicking Copy', async () => {
    const suggestions = [
      { reason: 'DB not ready', commands: ['make migrate', 'make cricsheet-import'] },
    ];
    render(<OpsSuggestions suggestions={suggestions} />);
    const btn = screen.getByRole('button', { name: /copy/i });
    fireEvent.click(btn);
    expect(global.navigator.clipboard.writeText).toHaveBeenCalledWith('make migrate && make cricsheet-import');
  });
});
