...
date is this run's data?" without loading the run. **Closed by P2-2, PR #280** (2026-09-07):
`RunManifest` carries `ratings_through` beside `cutoff`, required, written by retrain from
the state it built and asserted against that state at load — a manifest without the field,
or one whose date disagrees with its state, is refused by name, never served with a date
read from somewhere else; `/artifacts/status` answers the date for every run on disk and
names the reason a run cannot be loaded. The served run this audit measured
(`20260903T160602Z-0e1e39c2`) and every run written before the field are unloadable now,
by design: retrained, not patched. (4) Two freshness rules with different
...
1. **One freshness verdict, everywhere.** ✅ **Demonstrated in P2-1** (2026-09-07,
   PR #279): P0-4's disagreement was reproduced on the running build —
   `db_freshness.overall` read *stale* (TEST's latest match 2026-08-27, 11 days) while
   `artifacts.ratings` on the same payload read *fresh* at 5 of 14 — and the reconciled
   surfaces read one verdict with the lag beside it as a fact; with
   `XI_RATINGS_MAX_AGE_DAYS=3`, `/ops/status`, `/api/ml/xi-status` and
   `POST /api/predict/team-selection` refused together, naming 2026-09-02. The clause as
   written: every surface that shows freshness — the Health
...
2. **A run's data date is readable without loading it.** ✅ **Demonstrated in P2-2**
   (2026-09-07, PR #280): the retrain wrote `20260907T062657Z-6b16045e` with
   `cutoff 2026-09-07` and `ratings_through 2026-09-02`; the manifest, `/artifacts/status`,
   `/xi/status` and a served prediction all read the same date, and a manifest with the
   field removed was refused `409` by name. The clause as written: every manifest written by
...
3. **Every issued prediction is stored, verbatim.** ✅ **Demonstrated in P2-3**
   (2026-09-07, PR #282): the answer is encoded once, so the stored bytes and the served
   bytes are the same object; a store failure reached the wire as `record.stored: false`
   on the answer it failed to record, and the prediction was served. The clause as
   written: each successful
...
4. **Resolution needs no operator step.** ✅ **Demonstrated as mechanism in P2-4, not on
   a real fixture** (2026-09-07, PR #283): on the scratch database, through the real
   handlers, inserting the match a pending prediction was about moved it to `scored` on
   the next read with nothing else run. No prediction has resolved on `cricket_data` —
   the closure below says why, and what that leaves. The clause as written: after an
   import brings the match a stored
...
5. **Misses are on the record.** ✅ **Demonstrated as mechanism in P2-4, not on a real
   fixture** (2026-09-07, PR #283): on the scratch database the miss went on the record —
   a 0.6 for a side that lost, Brier 0.36, the record's mean 0.26 — and on the dev stack
   the two real forecasts read `unresolved` at −3 and −5 days with every scored number at
   n = 0, no result invented. Nothing real has been scored — the closure below. The
   clause as written: the record shows Brier and the reliability curve over
...
6. **Manual ingest fails honestly.** ✅ **Demonstrated across P2-1 and P2-2**
   (2026-09-07): the refusal with the date, the age against the limit and the remedy
   (P2-1, under `XI_RATINGS_MAX_AGE_DAYS=3`), and an import → retrain → reload run by hand
   serving the new run `20260907T062657Z-6b16045e` (P2-2); P2-4's record then carried the
   18 predictions P2-3 had issued and the one issued under the served run, 19 in all,
   none lost. The clause as written: with the ratings past the limit, the Lab refuses
...
**Where the gate stands (closed 2026-09-07): functional acceptance on mechanism, recorded.**
All four items shipped (P2-1 #279, P2-2 #280, P2-3 #282, P2-4 #283), in the order § 5.2 set,
with B-13's fix (#281) between the second and third because P2-2 found it. Each of the six
clauses above is marked with the item that demonstrated it. The user has closed the phase
on that evidence, and this passage says exactly what the evidence is, because two of the
six clauses rest on less than the other four and the record must not read as if they did
not.

**Clauses 1, 2, 3 and 6 are demonstrated on the dev stack, as the gate asked.** P2-1
reproduced P0-4's disagreement on the running build — `db_freshness.overall` *stale* on
TEST's 2026-08-27 latest match, 11 days, against `artifacts.ratings` *fresh* at 5 of 14 on
the same payload — and the reconciled surfaces read one verdict with the lag beside it as
a fact; with `XI_RATINGS_MAX_AGE_DAYS=3`, `/ops/status`, `/api/ml/xi-status` and
`POST /api/predict/team-selection` refused together naming 2026-09-02 (clauses 1 and 6).
P2-2's retrain wrote `20260907T062657Z-6b16045e` with `cutoff 2026-09-07` and
`ratings_through 2026-09-02`, the manifest, `/artifacts/status`, `/xi/status` and a served
prediction all read that date, and a manifest with the field removed was refused 409 by
name; the import → retrain → reload cycle, run by hand, served the new run (clauses 2 and
6). P2-3 encodes the answer once so the stored bytes and the served bytes are the same, and
a store failure was shown on the wire as `record.stored: false` without refusing the
prediction (clause 3).

**Clauses 4 and 5 are proven as mechanism, not on real data, and that is the whole
difference.** No prediction has resolved on `cricket_data`: on 2026-09-07 Cricsheet's
newest matches are dated 2026-09-02, the database's own latest day, so there was nothing
to import and no stored fixture could have been played and scored. Both clauses were
demonstrated instead **through the real handlers against the scratch database** — an
inserted match moved a pending prediction to `scored` on the next read with nothing else
run, a 0.36 miss went on the record and the record's mean Brier read 0.26 — while the dev
stack shows the two real forecasts as `unresolved` at −3 and −5 days, and every scored
number at n = 0, rather than a result it does not have. So resolution without an operator
step and misses on the record are demonstrated to work; **no real fixture has yet resolved
through them**, and the gate is closed on that basis by the user's decision. The first
real resolved fixture is a thing to look at when it arrives — the state it lands in and
the score beside it, read against what the scratch run showed — and not a pending
obligation; nothing here waits on it.

What was accepted, stated at its actual size: **the operating machinery of an internal
prototype, doing what this section says it does.** That is the whole claim. It is **not**
evidence that calibration holds: the record holds 19 predictions and none is scored, and
the paragraph above already says a Brier over tens is a diagnostic, not evidence —
`make evaluate` remains where a choice-facing number comes from, and a record that
disagrees with the harness goes to `docs/BUG_BACKLOG.md`, not to a verdict on the model.
And, as Phase 1's closure said of itself, it says nothing about users, retention, pull or a
market: route (a) left all of those unmeasured (§ 2), and closing this gate does not touch
them.

Left open beside the closure, and unaffected by it: **B-11** — the simulator fits one
dispersion to two populations, and its 10–90 interval is wrong in opposite directions by
day and at night; both gated candidates (`SIM-DN-split`, `SIM-DN-scale`) recorded nulls,
and the next candidate, a dispersion term the two innings do not share, is named in
`docs/BUG_BACKLOG.md` and plan §8.13. The record's coverage numbers will show that miss
once there is anything to cover, and must (§ 5.1).
...
#### P2-2 — The run manifest records `ratings_through` (model: Fable) — **shipped**
...
#### P2-3 — The prediction store: every issued prediction persisted, verbatim (model: Opus) — **shipped**
...
#### P2-4 — The internal track record: resolved predictions scored, misses included (model: Fable) — **shipped**
...
| P2 | ✅ **closed — functional acceptance on mechanism, recorded 2026-09-07.** All four items shipped (P2-1 #279, P2-2 #280, P2-3 #282, P2-4 #283) in the order P2-1 → P2-2 → P2-3 → P2-4, with B-13's fix (#281) between P2-2 and P2-3 because P2-2 found it; each of § 5's six clauses is marked with the item that demonstrated it. **Four clauses are demonstrated on the dev stack:** (1) P2-1 reproduced P0-4's disagreement on the running build — `db_freshness.overall` *stale* on TEST's 2026-08-27 latest match, 11 days, against `artifacts.ratings` *fresh* at 5 of 14 on the same payload — and the reconciled surfaces read one verdict with the lag beside it as a fact, refusing together under `XI_RATINGS_MAX_AGE_DAYS=3` (`/ops/status`, `/api/ml/xi-status`, `POST /api/predict/team-selection`) and naming 2026-09-02; (2) P2-2's retrain wrote `20260907T062657Z-6b16045e` with `cutoff 2026-09-07` and `ratings_through 2026-09-02`, read identically off the manifest, `/artifacts/status`, `/xi/status` and a served prediction, and a manifest without the field was refused 409 by name; (3) P2-3 encodes the answer once so the stored bytes and the served bytes are the same, and a store failure reached the wire as `record.stored: false` without refusing the prediction; (6) the refusal with the date, the age and the remedy (P2-1) and the import → retrain → reload cycle, run by hand, serving the new run (P2-2). **Two clauses are demonstrated as mechanism only, and the record says so:** clauses 4 and 5 — resolution without an operator step, and misses on the record — were shown through the real handlers against the scratch database (an inserted match moved a pending prediction to `scored` on the next read with nothing else run; a 0.36 miss on the record, mean Brier 0.26), because no prediction has resolved on `cricket_data`: on 2026-09-07 Cricsheet's newest matches are dated 2026-09-02, the database's own latest day, so there was nothing to import; the dev stack shows the two real forecasts as `unresolved` at −3 and −5 days and every scored number at n = 0 rather than an invented result. **No real fixture has yet resolved through them**, and the gate is closed on that basis by the user's decision. **What was accepted, at its actual size: the operating machinery of an internal prototype, doing what § 5 says it does.** **What was not:** calibration — the record holds 19 predictions, none scored, and a Brier over tens is a diagnostic, not evidence; `make evaluate` remains where a choice-facing number comes from — and, as with Phase 1, users, retention, pull and a market, all unmeasured under route (a). Open beside it and unaffected by the closure: **B-11**, the simulator's day/night interval defect — two candidates gated and nulled (`SIM-DN-split`, `SIM-DN-scale`), the next, a dispersion term the two innings do not share, named in `docs/BUG_BACKLOG.md`. The first real resolved fixture is a thing to look at when it arrives, not a pending obligation. Dropped with the reason recorded in § 5, unchanged: the public track-record page, scheduled ingest, a licensed availability feed, a further availability item |
...
| P3-1 | ✅ **shipped** (2026-09-08, PR #289) — the auction record and the pool's distribution by role. **The rule first, because it is the item:** the module is valuation and projection and never XI-picking, and that is enforced rather than asserted — `TestAuctionEndpoints_NeverReachXiOptimize` drives all four write-and-read handlers through the client and asserts every path they called is `/xi/player-roles`; the integration suite's scripted ml-service fails on sight for any other path; and `opsContract.test.ts` asserts no auction source names `xi/optimize`, `win_probability` or `marginal_value`. No win probability, no marginal value and no "best XI" is on the surface, and the tab carries the record's sentence where the numbers are. **The record:** migration `0015` adds `auction` (format, buying side by opposition id, squad size, and the eleven's `min_bowlers` / `require_keeper` — the same two the predict path takes), `auction_venue` (the grounds as venue ids, a child table so the reference to `venue` is a real one) and `auction_player` (state `available | sold | unsold` held by a CHECK, buyer name and optional club id, price as an integer in the auction room's own unit with no currency implied, `state_changed_at`); a second CHECK refuses half a sale, and the comment says what it is — facts the operator types during a live auction — and what it is not: not a prediction, not a cache, not a feed. Nothing here is on the track record and nothing is ever scored, because no auction outcome data exists to score it against. `internal/auction` states the shape once, away from the SQL and the HTTP: the state vocabulary, a `Store` with its mockery mock, and the arithmetic (squad, open slots, distribution). **The roles are deliberately not stored** — they are what the served vectors say today, so freezing one run's answer into a record that outlives the run would leave the surface showing a role with no date on it. **The endpoints:** create, list, read whole, add players, record an outcome — every write answering with the auction as it now stands, and an undo being the outcomes call with `available`, which clears the buyer and the price with the state. `GET /api/players/search` is the cross-club name search an auction list needs (`/api/options/candidates` is per club because a prediction is about one side); it reuses the candidate list's appearance unions and its `COALESCE(canonical_id, id)` club grouping, applies no recency window and removes nobody — the retirement ledger's verdict is shown beside a name, not instead of one. **The roles, from the model — the substance of the item:** the two predicates were written out three times (`optimizer._Pool`, `xi_service._constraint_check`, and again where `selection_reasons` names them) and are now one definition in **`ml/xi/roles.py`**, which all three call. `POST /xi/player-roles` is the fourth reader — a format and up to 500 ids, back per id whether the served vectors say keeper and bowling option, stamped with `served_ratings` — and it exists as its own route because the auction asks about players who are in no eleven at all, which `selection_reasons` can never answer. **A test pins this route's answer equal to `selection_reasons`' player for player on the same run**, which is what makes the extraction more than a tidy-up. An id the served state has never seen is `known: false` with no roles and is named in `unknown_player_ids`; H-11 refuses the whole read past the limit. go-app puts the roles on each row with `run_id` and `ratings_through` beside them, a player answering neither predicate is a **batter by elimination** and the glossary entry says that is what the label means, and `player.is_wicket_keeper` appears only on the search, named as the database's own name-set flag. **The surface:** an Auction tab (`/auction`) with the list, the squad and open slots (total, and by constraint), and the remaining pool's distribution by role, through `RatingsAsOf` and `PredictionReadiness`. Nothing is derived in the browser — every count arrives on the answer, because a count computed there would be a second definition of a number the surface is reporting. Both labelled numbers are L-1 keys (`auction_open_slots`, `auction_available_by_role`) whose explainers state what the counts are not: not a selection, not a projection, and overlapping rather than a partition. **H-24:** `auction_player_states` and `auction_metric_keys` join the generated contract, asserted from both sides; the roles are not a new vocabulary but `selection_roles`, which is the point. **Verified on the dev stack (2026-09-08, D-6 way; branch processes on 8001/8081 beside the containers, serving `20260908T051956Z-626dc507`, ratings through 2026-09-02):** a T20 auction for Chennai Super Kings, squad 25; twenty real players listed through the search across CSK, MI, RCB, KKR, GT, DC, LSG, RR, PBKS and international sides, the served vectors making Dhoni / Pant / KL Rahul keepers, ten bowling options and seven batters by elimination; sales to three buyers (Dhoni 1200 and Jadeja 1400 to CSK, Bumrah 1800 to Mumbai Indians, Rashid Khan 1500 to Gujarat Titans), two unsold and one undo, with the counts moving on every entry — available/keepers/bowling options/batters `20/3/10/7 → 19/2/10/7 → 18/2/9/7 → 17/2/8/7 → 16/2/7/7 → 15/2/7/6 → 14/2/7/5 → 15/2/8/5`, open slots 25 → 24 → 23, `keeper_needed` false on the Dhoni sale and `bowling_options_short` 5 → 4 on the Jadeja sale. **The reload is exactly what was entered:** squad `[388, 603]`, three sold rows with buyer and price, two unsold, Rashid Khan back to `available` with both cleared. **Unknown:** a listed player the state has never seen reads `known: false` and is counted apart from the batters. **Stale** (branch ml-service with `XI_RATINGS_MAX_AGE_DAYS=1`): the read answers 200 with `roles {available: false, code: "RATINGS_STALE", message: "ratings run through 2026-09-02 (6 days old, limit 1)"}`, no distribution and no slots-by-role, the 21-row list and the three sold rows intact, and `slots.open` 23 still shown because it needs no model. **One defect found in the run and fixed:** a new auction with nobody on it was claiming `roles.available: true` with no run and no date and a distribution of all zeroes — which reads exactly like a served answer with no keepers left; it now reports `ROLES_NOT_READ` and drops every count off the model. **Blast radius:** ml-service gains one route and one module, the three call sites of the predicates delegate rather than restate them and no predicate changed value; the harness, the run plans and the three pipeline steps are untouched and `make evaluate` was not run — this item produces no choice-facing number. `ARCHITECTURE_MAP.md` regenerated, `make check-system-map` agrees in both directions. `make check-all` green; **coverage gates never moved down** — go-app stays at 78 (CI run 34266207574: `Coverage 78.9% meets threshold`), ml-service at 93 (CI run 34266207608: `Total coverage: 93.79%`), and the frontend ratchets **lines 82 → 83, functions 81 → 82, statements 80 → 81** against CI run 34266207576's own figures (`All files | 81 | 73.92 | 82.76 | 83.23`), branches staying at 73 (B-9). `cricket_data` unchanged at 22,818 / 11,539,808 / 13,662; the only rows this item wrote there are 2 `auction` and 21 `auction_player`. **The dev stack as left:** the containers were never rebuilt and still serve main's images; the branch processes were stopped |
| P3-2 | ✅ **shipped** (2026-09-08) — the projection: a candidate's output in the buyer's likely eleven, against a named opposition, at the buyer's grounds. **The rule first, because it is the item:** valuation and projection, never XI-picking. `/simulate` answers a `win_probability`; the Go type this endpoint maps it into (`internal/server/ml_auction_client.go`) has no such field, so the value is dropped at the JSON boundary and never exists in the process at all. `TestProjectAuctionCandidate_CarriesNoWinProbabilityAndNoMarginalValue` walks every key of the rendered payload at any depth and refuses one containing `win` or `marginal_value`, asserts the simulated 0.61 the scripted service answered is nowhere in the bytes, and asserts the paths the module reached are exactly one `/performance/predict` and one `/simulate` per ground and never `/xi/optimize`. **The three inputs are assumptions, and named.** Migration `0016` adds `auction_likely_xi`, `auction_opposition` and `auction_opposition_player`, so every item after this reads one list rather than its own guess at one; `PUT /api/auctions/{id}/assumptions` writes them (each field optional, an absent one left as it stands, a named one replaced rather than merged), and every answer carries the eleven, the opposition and the grounds back. `GET /api/auctions/{id}/opposition-suggestion?club_id=` offers the eleven this database records a side last fielding in the format, with the match's date beside it, as a starting point to edit — and only a *complete* eleven, since a partial team sheet would seed an assumption with a hole in it; where none exists it is `404 NO_FIELDED_ELEVEN` rather than a side assembled by rating. **The opposition carries a club id and not only eleven names, and that is load-bearing:** `ml.xi.rows.team_context_or_neutral` falls back to neutral for the *pair* and takes the venue with it, so without a named side every ground would read identically and the venue mix this item exists for would be three copies of one row. **The projection.** `POST /api/auctions/{id}/projection` returns, per ground, the candidate's `runs`, `balls_faced` and `runs_conceded` as L2-B's `q10/median/q90` with the toss marginalised unless the operator sets it, his `wickets` as an expectation with `p0`/`p1`/`p2_plus` and **no interval at all** (L2-B has no quantile heads for wickets on this path, and one derived from the pmf would be an interval the model never made), and the eleven's total with him in it from `/simulate` at the served 2000 draws with his `spread_share`. **Batching is impossible and the PR says why from the code:** `ml.xi.rows.player_feature_rows` builds each row's `own_*` columns from `aggregate_side` over the whole side it is handed, so listing N candidates on one side would project each into a side of 10 + N rather than the eleven he would join, and `/simulate` draws exactly the innings it is given. **Two intervals, told apart.** `auction_interval_sources` (`l2b_quantiles` | `simulator_draws`) joins the generated contract and is asserted from both sides (H-24); each number carries its source, the answer explains both once, and the simulator's names **B-11** (too narrow by day, too wide at night — T20 first-innings coverage 0.734 / 0.841 at a nominal 0.80, six gated arms nulled) and **B-14** (the performance artifact is not shape-checked at load, so an older calibration can restore silently: the run id shown is the run that answered, not a promise about its calibration). Nothing is widened, narrowed, adjusted or hidden. **The mixture is over draws, never over quantiles** — a mixture's quantiles are not the mean of its parts', so `/simulate` gained an opt-in `return_total_draws` and `auction.MixtureQuantiles` inverts the pooled empirical distribution; the pure function is tested against sixteen hand-computed draws whose mixture median is 172 where the mean of the two grounds' medians would be 170, a total neither ground drew. Without weights there is no mixed range, because how often an eleven plays where is a fact nobody has entered. **What a ground changes, on the answer:** `venue_context` (`venue_bf_rate`, `venue_n`, `neutral`) is new on `/performance/predict`, read off the rows the model consumed rather than recomputed, and the answer says in the code's own terms that a ground moves the rows through those two columns and nothing else (A-1's fixture-context families were nulled, `FIXTURE_CONTEXT_FAMILIES_KEPT` is empty); a ground the served state has no matches at reports `neutral: true` with a note saying it read at the prior (§8.7). **Verified on the dev stack (2026-09-08, D-6 way; the branch's own images rebuilt into the compose project, serving `20260908T051956Z-626dc507`, ratings through 2026-09-02):** a T20 auction for Chennai Super Kings at Eden Gardens (136), the Wankhede (163) and the Chinnaswamy (166); a likely eleven of ten (Dhoni, Jadeja, Pandya, Gill, Rohit, Rahane, Moeen, Rashid, Boult, K Yadav) against Punjab Kings' last recorded eleven (2026-04-16, from the suggestion endpoint); three candidates by the model's own role read — **RR Pant** (keeper), **JJ Bumrah** (bowling option) and **V Kohli** (a batter by elimination). **The surface's numbers are the stack's, checked against ml-service directly for Pant at Eden Gardens:** runs `0.00 / 17.2258469 / 56.5379685`, balls faced `0.00 / 13.5977961 / 34.8143485`, `venue_context` bf `0.42346939` over n `93`, total `130 / 178 / 236`, `spread_share 0.11073822` — identical on both, with `run_id` and `ratings_through` on every one. The same call's `/simulate` answered `win_probability {simulated 0.547, display 0.5805}`, which is nowhere in the endpoint's payload. **The venue mix is small and honestly so:** Pant's totals read 130/178/236, 131/179/235 and 131/178/235 across the three grounds, and his own quantiles are identical to five decimal places — exactly what § 6 predicts, since the grounds differ only in bat-first rate (0.4235 / 0.4398 / 0.4254) and n (93 / 103 / 62). **The toss, both ways at the same ground:** batting first runs median 18.00 and total 138/188/252 (spread 0.1945), chasing 16.45 and 125/168/223 (spread 0.1495), with the marginalised 17.23 sitting between them. **A neutral ground:** venue 9, which the served state has no T20 matches for, reports `neutral: true`, n 0, bf 0.5000 and the note, beside Eden Gardens on the same answer. **Refusals, each showing no number:** nine named → `400 XI_INCOMPLETE` *"holds 10 players, and an eleven is scored as 11"*; eleven named plus an outside candidate → the same code, *"holds 12 players"*; H-11's limit lowered to 1 → `503 RATINGS_STALE` *"ratings run through 2026-09-02 (6 days old, limit 1)"*, while the record itself still reads back whole with all 21 rows and both assumptions and only the model's counts dropped. **The cost, measured and recorded because P3-3 and P3-5 pay it per available player** (30 timed repeats after 5 warm-ups): **312.1 ms median / 358.3 ms p95 per candidate per ground**; 967.3 / 1215.1 ms for one candidate over three grounds; 971.8 / 1067.0 ms with the mixture, which is therefore free. Of one ground, ml-service is 138.3 ms (`/performance/predict`) and 173.5 ms (`/simulate`); `return_total_draws` costs 171.4 ms, inside the noise. **No cache was added** — if one is warranted that is P3-5's decision, made with these numbers. **Blast radius:** two additive serving fields and nothing else in ml-service — neither is a feature, neither enters a fit, no measured number moves, and `make evaluate` was not run because this item produces no choice-facing number. Four glossary entries join L-1 (`auction_projected_output`, `auction_projected_total` and the two interval sources), and the surface reuses the Lab's `spread_share` rather than minting a second explanation of one quantity. `ARCHITECTURE_MAP.md` regenerated with a new `auction-projection` node; `make check-system-map` agrees in both directions. `cricket_data` unchanged at 22,818 / 11,539,808 / 13,662; the rows this item wrote there are three verification auctions with their lists and assumptions. **Coverage gates never moved down, and two ratcheted on CI's own figures (B-9).** The frontend's branches gate moves **73 → 74** and go-app's **78 → 79**, both on PR #290's own runs — frontend run 34275028302 measured `All files | 81.34 | 74.23 | 82.71 | 83.54` and go-app run 34275028287 measured `Coverage 79.1% meets threshold`, each identical to the local figure; ml-service's run 34275028261 measured 93.80 %, which rounds down to the 93 it already sits at. The frontend's other three gates were already at their figures rounded down. |
