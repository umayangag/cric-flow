...
| H-4 | **Monotone objective.** Upgrading a player never lowers the selection score | win objective | Harness: share of one-player upgrades with Δp < 0 must stay < 2% (measured 0.4% T20 / 3.7% ODI for logistic; 12% / 20% for unconstrained boosting) | **done (P-2)** — reported per fold in `make xi-evaluate`; walk-forward means 0.3% T20 / 0.8% T20I / 0.0% ODI / 0.6% TEST, all under the 2% line. **The line covers the objective, and the display surface is now exactly monotone under the same probe** (B-7): `display_swap_violation_share` reports **0.0000 in every format and every fold**, where it measured 5.1% T20 / 7.1% T20I / 3.4% ODI / 6.1% TEST before. Still reported and not gated — H-4's contract is the objective's — but nothing is left uncontracted on the surface a person watches. Getting there took two readings. Constraining the display model's team-context columns was gated (`B-7-display-monotone`) and is a **recorded null** that moves the violations the wrong way; it never could have worked, because the probe holds team context at the fixture's values. The whole effect is `pelo_std`, the one column an upgrade moves that the contract leaves free, and the informing gate `B-7-pelo-spread` priced its removal: exactly 0.0000 violations for −0.0004 (T20), **+0.0088 (T20I)**, −0.0055 (ODI), −0.0047 (TEST) of display AUC. **That trade was taken by decision** — `DISPLAY_FEATURE_COLS` no longer carries `t1_pelo_std` / `t2_pelo_std` (`contract.DISPLAY_EXCLUDED_COLS`) — because the Team Lab's what-if is the product and a swap that moves the probability the wrong way one time in twenty undermines it. **It was not free: display AUC in ODI and TEST is about half a point lower**, and `docs/BUG_BACKLOG.md` § B-7 records the cost beside the zero. The objective is unchanged and keeps the column. **Enforced (EVAL-04):** the 2 % line is a `Threshold` beside the gate's `report_path` in `ml/xi/gates.py`, evaluated by `gates.check_report` on the walk-forward mean in every format the folds scored, and `make evaluate` exits 1 when any format reaches it |
...
| H-14 | **Noise floor.** Differences under the measurement's own noise are not evidence. For a paired comparison over the walk-forward folds the floor is one fold-level standard error of the paired difference; for a single holdout, one Hanley–McNeil standard error of the AUC. A spread across random seeds is **not** a floor: the display model's seed reached only sklearn's early-stopping split, the three seeds it was fitted under were the same fit below 10,000 rows, and the spread reported across them was identically zero (EVAL-02). The rule as first written — "every reported number is a mean over ≥ 3 seeds with the spread" — measured nothing for the win models | all | The harness fits each model once and reports mean ± spread over folds; `gates.py` and `run_usability.py` read the standard errors above. The performance model's members keep their seeds, which do enter through its explicit early-stopping split | done — restated under EVAL-02 |
...
| Reported with uncertainty | walk-forward mean ± spread across cutoffs, with the fold-level paired standard error as the floor (H-14; the seed spread was retired under EVAL-02, having been identically zero) | done |
