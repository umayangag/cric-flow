...
**go-app already had the date and never sent it.** `predictteam.Input.MatchDate` is the day the caller typed; it reached the pool's recency cutoff and `asOfFor` (which sets `as_of` only for a *past* match) and stopped there. It is now threaded through `fixture.matchDate` onto both payloads, so the live case — a fixture today or later, where `as_of` is empty and the old default was worst — is the case that is fixed. The frontend already sends `match_date` to go-app and needed no request change.
