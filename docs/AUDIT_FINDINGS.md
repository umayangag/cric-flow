...
`runs.py:153-174` shells out to `git rev-parse HEAD`; the image (`ml-service/Dockerfile:11-35`) has no git and no `.git`, so `git_sha=""` on every served run. Missing: library versions, objective `C=0.3`, display `l2_regularization` / `min_samples_leaf`, `DISPLAY_SEEDS`, the performance `FitSpec`, source type. `dataset_sha` hashes `match_id|match_date` of decided matches only, so a re-import that changes squads or deliveries yields an identical sha. **Fix.** Inject `GIT_SHA` as build arg/env; add `library_versions`, `FitSpec.as_dict()` and the win-model constants; extend the digest with per-match delivery/squad counts. — PR #PRNUMBER.
