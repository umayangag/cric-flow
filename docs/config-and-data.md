...
- **One reduced day per (venue, match day)** — `reference-data/era5-venue-days.jsonl`: the
  ...
  first day. The readings are asked for **in UTC** and put on the venue's clock here, hour
  by hour, from the IANA zone the table holds: the service's own `timezone=auto` stamps a
