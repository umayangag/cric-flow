...
| S-10 | done | `select/s-10-xi-win-model` | **XI-responsive win model**, then behind `selection.win_model: "xi"` — DB acceptance run; the model reproduces its JSON-path numbers (0.72–0.75 objective, 0.72–0.75 display); the S-3b selection gate did not pass, though that run has since been shown to have measured other arms (plan §8.5). **P-5 removed the flag**: the XI path is the only selection path |
...
| S-6 | **shipped** | `arch/p-5-repoint-surfaces` | Turn on win-probability selection. **Shipped in P-5 on a replacement gate.** The original gate reported pooled winner accuracy 0.560 (`winprob`) vs 0.569 (`greedy`) over 332 locked-window matches, and was **later shown never to have run the `winprob` arm** (plan §8.5, D-7a): the broken id contract left every player unrated, so nobody was a bowling option, `/xi/optimize` refused every limited-overs call on `min_bowlers`, and go-app fell back to the windowed-form optimiser without surfacing it. Re-run on 2026-09-02 with the contract fixed, over 357 locked-window matches: `winprob` 0.622 ± 0.026 against a rating-ordered baseline's 0.625 ± 0.026, with the actually-fielded XIs highest at 0.650 — one match apart, and still not an answer, because the gate is unwinnable by an arm that optimises *both* sides: it asks whether the predicted winner of a **counterfactual** fixture matches the result of the real one, and strengthening both sides moves that fixture toward parity (mean \|p − 0.5\| 0.153 optimised vs 0.180 greedy). P-5 re-measured the replacements the plan named, on the database (plan §8.4): swap violations **0.3 %** in T20 (0.0–0.8 % across formats) against a 2 % line, and specific-XI-beyond-typical-XI **+0.012 ± 0.010** in T20 — positive but about 1.2 sd from zero, and inside its own noise in ODI and T20I. That is weaker than the +0.045 ± 0.021 the plan previously recorded; P-5 ships selection on the XI objective for T20 / T20I / ODI on the measured figure, not the earlier one. There is no flag: the XI path is the only selection path, and TEST is served a rating-ordered XI marked not optimised (H-17). **E5, the gate meant to settle this, was run for all three formats after the switch shipped (plan §8.6) and splits them**: its as-played form is confounded, and the form that isolates the lineup gives T20I 0.589 ± 0.016 (0.55 below the interval — earned), ODI 0.521 ± 0.010 (above chance, line outside) and T20 0.509 ± 0.007 (excluded). Nothing here argues for restoring a flag, since the arm it would switch to was deleted, but the accurate summary is that the objective is a good win model everywhere it is served, a demonstrated selector in T20I, and an unproven one in T20 and ODI |
