...
| **`ml-service/ml/fill_missing_attributes.py`** (removed) | Missing attributes in player pool | Was `fillna(column.mean())` then `fillna(0)`. | Was used only by removed `export_pool` script. See `ml-service/docs/DEAD_CODE_ANALYSIS.md`. |
