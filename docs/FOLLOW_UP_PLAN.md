...
| `shared_factor_folds` | Folds with a shared match factor | How many walk-forward folds simulated with the shared match factor fitted, which windows did not, and the same totals over the folds that did. A calibration window holding too few complete first innings to deconvolve a factor ships the un-widened simulator, whose intervals are much too narrow, so the two figures say how far a thin fold pulled the pooled one. | Every scored fold, ideally. Where one is thin the pooled first-innings coverage sits below the calibrated folds' -- 0.756 against 0.774 on 10 of 11 ODI folds, 0.783 against 0.792 on 9 of 10 T20I folds, and all 11 T20 folds have a factor (B-12). | higher is better |
...
| `marginal_value` | Marginal value | The win probability the side loses if this player were replaced by a par player in his role: the same expected balls faced and bowled and the same keeper flag, but scoring and conceding exactly what the format expects off every ball, at the initial rating. His contribution, with the slot he fills held fixed. It is the reason he is in the eleven, stated as a number. | A few points of win probability across a typical XI. It is a ranking aid, not a promise. | higher is better |
...
X-4's market benchmark added these (EXTERNAL_DATA_PLAN.md § X-4). Every one of them is
read beside the joined coverage printed with it, because the matches they score are a
small and different population from the format's headline numbers:

| key | plain-language name | what it means | reference | better |
|---|---|---|---|---|
| `market_auc` | Market AUC | The same measure applied to the betting market's closing price, de-vigged into a probability. It is the best publicly observable forecast of the same match, so it is the yardstick the display model is held against. Of two random opposing claims, how often the model ranks the actual winner higher. 0.5 is a coin flip and 1.0 is perfect. | Read it beside the joined coverage printed with it, and beside the other arm on the same matches -- never on its own. It is a different, usually much smaller, set of matches than the format's headline numbers. | higher is better |
| `market_brier` | Market Brier | The same squared-error score for the market's de-vigged closing probability. Squared error of the stated probability, averaged over matches. It rewards a probability that is both right and confident, and punishes confident and wrong. | Read it beside the joined coverage printed with it, and beside the other arm on the same matches -- never on its own. It is a different, usually much smaller, set of matches than the format's headline numbers. | lower is better |
| `display_toss_aware_auc` | Display AUC, toss-aware | The displayed model read at the batting order that actually happened, rather than averaged over both. The served probability marginalises over the toss because the toss is unknown when a user asks; the closing market price is struck after it. This arm gives the market's information set to our model, so the comparison is like for like. | Read it beside the joined coverage printed with it, and beside the other arm on the same matches -- never on its own. It is a different, usually much smaller, set of matches than the format's headline numbers. | higher is better |
| `display_toss_aware_brier` | Display Brier, toss-aware | The same squared-error score for the toss-aware reading of the displayed model. | Read it beside the joined coverage printed with it, and beside the other arm on the same matches -- never on its own. It is a different, usually much smaller, set of matches than the format's headline numbers. | lower is better |
| `market_minus_display_auc` | Market minus display, AUC | How much better the market ranks the same matches than the probability a user is shown. Positive means the market is ahead; it is the distance to the practical ceiling, in AUC. | Zero is parity with the market. Read it through the 95 % interval beside it: an interval spanning zero means the two are indistinguishable on the matches scored. | lower is better |
| `market_minus_display_brier` | Market minus display, Brier | The same gap in Brier score. Negative means the market's probabilities are better stated than ours; positive means ours are. | Zero is parity with the market. Read it through the 95 % interval beside it: an interval spanning zero means the two are indistinguishable on the matches scored. | higher is better |
| `market_minus_toss_aware_auc` | Market minus display, AUC, toss-aware | The same gap against the toss-aware arm -- the honest one, because both sides then know who batted first. | Zero is parity with the market. Read it through the 95 % interval beside it: an interval spanning zero means the two are indistinguishable on the matches scored. | lower is better |
| `market_minus_display_auc_ci95` | 95 % interval of the market-minus-display AUC gap | The range the gap could plausibly be, from resampling the scored matches in pairs (both arms score the same matches, so the resample keeps them together). An interval spanning zero means the two are indistinguishable on this many matches. | Read the gap only through this interval; one spanning zero is not a gap. | no good direction alone -- read it beside its pair |
| `market_minus_toss_aware_auc_ci95` | 95 % interval of the toss-aware AUC gap | The same interval for the like-for-like comparison. | Read the gap only through this interval; one spanning zero is not a gap. | no good direction alone -- read it beside its pair |
| `market_overround` | Market overround | The two sides' implied probabilities added up before de-vigging. On an exchange the excess over 1.0 is the back/lay spread rather than a bookmaker's margin, which is why it is small. | About 1.00 on an exchange; a bookmaker's would be 1.05 or more. Reported so the de-vig is auditable. | no good direction alone -- read it beside its pair |
| `joined_share` | Joined coverage | The share of the format's evaluated matches that a closing price could be joined to. It is part of the benchmark's answer, not a footnote: a market comparison over a tenth of a format says what it says about that tenth. | 0.0 means the benchmark says nothing about this format. Read every market number beside it. | higher is better |
...
| A-3 | **done — a recorded null, per family** — `feat/a-3-t20-lineup-signal`, stacked on A-2. Gate A-3 (H-23 triple registered first: *varies* the feature family the selection objective reads, *fixed* everything else including E5's pairs and the bar's derivation, *decides* T20's fold lineup-only E5 against the bar re-derived from each arm's own claimed effect under three seeds, with a no-degradation guard on AUC and H-4 in every format) tried the three candidates one at a time on the folds (plan §8.11): **(a) phase matchup** — Cricsheet carries no bowling style, so the spin/pace split is not constructible and the per-phase batting and bowling impacts with same-phase cross terms are the matchup axis — clears T20's bar by 0.004 (0.510 ± 0.011 against 0.505–0.506, under half a standard error, +0.007 over the control) and is not shippable: it breaks H-4 in ODI (25 % of one-player upgrades lower P(win), the phase columns being collinear with the base impacts), lowers TEST's AUC by 0.007 ± 0.006 and drops T20I's agreement 0.564 → 0.486 on the one format where the objective is a proven selector. **(b) role balance** fails outright (0.496 against 0.504–0.506) and puts T20's own swap share over H-4's line (2.9 %). **(c) reweighting E5 by the claimed \|Δ\|** is a change of the measurement, not the model, and is dropped as such: T20's agreement rises with the claimed \|Δ\| (0.476 / 0.495 / 0.535 over the terciles, the rotation pairs at chance) but every tercile sits under its own exactly-right expectation, and the reweighted readings pass their re-derived bars by 0.001–0.005 on the control — inside noise, after the number was seen. The control reproduces the harness (0.503 over 2,169 pairs against 0.505–0.506; parity 0.0 on the reconstruction and on the previous elevens). T20 stays rating-ordered; the reason on the wire now quotes A-4's figure and says A-3 did not change it. `make evaluate` after the choice on both sources (3 h 35 min each, in parallel): the walk-forward table is A-4's baseline to every value compared — only fit and latency timings differ from A-2's post-choice reports — the locked window still holds 0 matches, H-8 parity 0.0 on both sources, gates (A-3 registered) and glossary pass, and the report's selection decision agrees with the served policy in every format. Judgment calls recorded in plan §8.11; no bug found. |
| A-4 | **done** — `chore/a-4-locked-window-rotation`. The locked window rotates to **2026-09-02** (the P-7 merge date, the line no decision has read past); the spent ≥ 2025-09-01 window retires into four new quarterly folds (2025-09, 2025-12, 2026-03, 2026-06), so the harness runs **eleven** folds, not seven, and the retired data is scored where reuse is allowed. The report carries `locked_window` (start, rotation date, previous start, reason, retired cutoffs) and repeats both dates on every locked figure; the Evaluation tab and the System map render them. **The new window is empty** — the database ends 2026-09-01, so it holds 0 matches and says so (`n_eval: 0`, `skipped_reason`) rather than reporting noise; it fills as the cadence runs (A-5). While it cannot fit a model, H-8's parity check serves the last fold's (`parity_model_window: 2026-06-01`) and still compares 50 matches / 1,100 predictions / 50 simulations at max diff **0.0**. Policy in `ml-and-training.md` § Rotating the locked window. **Baseline for A-1/A-2/A-3** below. Run: 67 min, gates and glossary pass. |
| A-5 | **done** — `chore/a-5-retrain-cadence`, stacked on `fix/reload-publishes-the-new-run` (#246) and A-3. The cadence is **one command**: `make cadence` starts the new `refresh` run plan — fetch → extract → import → retrain → reload — through `POST /ops/pipeline/run-plan`, waits for it, and turns the outcome into an exit code (`scripts/cadence.sh`; `make cadence-dry-run` checks the same preconditions and starts nothing). The plan is **composed** from `import` and `retrain-only` rather than restated, so the pipeline's shape stays the registry's; ordering, run history, progress and Stop are the server's, because a scheduler entry with its own copy of them would be a second pipeline. **Reload is the last step and the executor stops at the first failure**, so a bad retrain publishes nothing — including on the data-quality gate, which is the one thing in the chain that assumes a human: neither `/admin/train/retrain` nor the plan passes `--accept-data-quality`, so the unattended path can refuse to publish and cannot approve (tested at both ends: `test_run_retrain_never_accepts_a_data_quality_regression_on_its_own`, `TestExecute_RefreshPublishesNothingWhenTheRetrainFails`). **Weekly**, because Cricsheet's rhythm decides it (the archive fetched here carried `Last-Modified: Tue, 01 Sep 2026 22:28:22 GMT`) and H-11's 14-day limit leaves one missed run of slack — the age is measured from the last **match date**, not the last run. `deploy/cadence/` holds a systemd timer (`Persistent=true`, `RandomizedDelaySec=1800`) and a crontab, as documentation, enabled by nothing. **Measured end to end on this box: 11 min 22 s** — fetch 26 s, extract 21 s, import 1 min 15 s, retrain 9 min 3 s, reload 2 s — ending `current_run` = `loaded_run` = `20260903T160602Z-0e1e39c2`, `ratings_through 2026-09-02`, **age 1 day** against the 14-day limit. A first attempt failed at fetch (`download: unexpected EOF`): the plan stopped there, nothing after it ran, and the command exited 2 — the failure path, unrehearsed. **The item found B-2 and could not proceed without it**: `POST /admin/reload` with no run named loaded the run `current` already pointed at, so the reload after a retrain re-loaded the run already serving and the pipeline published nothing while every step reported success — two runs on this box had been built and never served, the database holding matches to 2026-09-01 while the served ratings ran through 2026-08-25. Fixed in #246 with the regression test that fails without it. B-3 recorded and not fixed: a retrain at today's cutoff has an empty holdout, so its manifest carries no per-format metrics. |
