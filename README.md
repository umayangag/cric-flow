...
Evaluation report tab: L4's walk-forward folds, both AUCs per competition level under them (which side of a pooled format a move came from), the locked window, the display-regression verdict per format (this run's display AUC against the previous accepted run's, judged only on the same population — see `docs/ml-and-training.md`), per-target performance with interval width beside coverage, the simulator's E2 section, the train/serve parity check and the market benchmark (X-4: closing odds scored beside the displayed probability, with the joined coverage printed beside every number — a yardstick, never a model input). It reads one file (`make evaluate` writes it) — there is no form, because the folds and the locked window are the harness's. See **docs/apis-backtest-and-ops.md**. Start stack: `make dev-up`; frontend: `make frontend-dev` (http://localhost:5173). API key in UI: the `API_KEY` the backend was started with; there is no default, and `docker compose` refuses to start without one (see `.env.example`).
