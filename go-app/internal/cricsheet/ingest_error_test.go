...
func TestImportDir_ErrorHandling(t *testing.T) {
	...
	require.NoError(
		t,
		os.WriteFile(file1, []byte(`{"info":{"match_type":"T20","teams":["A","B"],"dates":["2024-01-01"]}}`), 0o600),
	)
	require.NoError(
		t,
		os.WriteFile(file2, []byte(`{"info":{"match_type":"T20","teams":["C","D"],"dates":["2024-01-02"]}}`), 0o600),
	)
	...
	t.Run("FailFast_Enabled", func(t *testing.T) {
		...
		mdb.On("EnsureMatchWithFormat", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).
