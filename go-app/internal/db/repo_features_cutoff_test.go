...
func setNullFloat64(ptr any, valid bool, val float64) {
    ...
    if rv.Kind() != reflect.Ptr || rv.IsNil() { return }
    v := rv.Elem()
...
func writeNullableFloat(dest []any, v1 bool, f1 float64, v2 bool, f2 float64) error {
    if len(dest) >= 1 { setNullFloat64(dest[0], v1, f1) }
    if len(dest) >= 2 { setNullFloat64(dest[1], v2, f2) }
    return nil
}

func writeSingleNullableFloat(dest []any, v bool, f float64) error {
    if len(dest) >= 1 { setNullFloat64(dest[0], v, f) }
    return nil
}
...
func TestPrecomputedFirst_OverridesFallback(t *testing.T) {
    ...
    feats := got[1]
    if feats == nil { t.Fatalf("missing features for player 1") }
    ...
    if feats["avg_runs"] != 10.0 { t.Fatalf("avg_runs = %v, want 10", feats["avg_runs"]) }
}
...
func TestCutoffArgs_PropagatedToFallbackQueries(t *testing.T) {
    ...
    for _, a := range fq.lastArgs {
        if tt, ok := a.(time.Time); ok { found = tt; break }
    }
...
func TestPartialData_ReturnsPartialMaps(t *testing.T) {
    ...
    if err != nil { t.Fatalf("unexpected err: %v", err) }
    feats := got[99]
    if feats == nil { t.Fatalf("missing features for player 99") }
    if _, ok := feats["avg_economy"]; !ok { t.Fatalf("expected avg_economy present") }
    if _, ok := feats["batting_form"]; ok { t.Fatalf("did not expect batting_form present") }
}
