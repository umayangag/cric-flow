...
func assertErrContainsG(sub string) assertFnG {
	return func(t *testing.T, err error) {
		...
		if err == nil || indexOfG(s, sub) < 0 { t.Fatalf("want err containing %q, got %v", sub, err) }
	}
}
...
func TestServices_Guards(t *testing.T) {
	...
	cases := []struct{
		...
		},
		{
			...
			act: func() error { s := svc.NewBattingService(nil); return s.ExportLegacy(context.Background(), &bytes.Buffer{}) },
			assert: assertErrContainsG("nil service or repo"),
		},
		{
		...
		},
		{
			...
			act: func() error { s := svc.NewBowlingService(nil); return s.ExportLegacy(context.Background(), &bytes.Buffer{}) },
			assert: assertErrContainsG("nil service or repo"),
		},
	}
