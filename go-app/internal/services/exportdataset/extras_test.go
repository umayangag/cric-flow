...
func TestExtrasService_Exports(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			act: func(ctx context.Context, s *svc.ExtrasService, w *bytes.Buffer, m *dbmocks.MockDatasetRepo) error {
				m.EXPECT().ExtrasUnifiedRows(mock.Anything).Return([][]string{{"h1", "h2"}, {"a", "b"}}, nil)
		...
		},
		{
			...
			act: func(ctx context.Context, s *svc.ExtrasService, w *bytes.Buffer, m *dbmocks.MockDatasetRepo) error {
				m.EXPECT().ExtrasFormatRows(mock.Anything, "ODI").Return([][]string{{"fh1", "fh2"}, {"x", "y"}}, nil)
				return s.ExportFormat(ctx, "ODI", w)
			},
			want:   "fh1,fh2\nx,y\n",
...
func TestExtrasService_Errors(t *testing.T) {
	...
	cases := []struct {
		...
		{
			"unified error",
			func(ctx context.Context, s *svc.ExtrasService, w *bytes.Buffer, m *dbmocks.MockDatasetRepo) error {
				m.EXPECT().ExtrasUnifiedRows(mock.Anything).Return(nil, errors.New("boom"))
				return s.ExportUnified(ctx, w)
			},
		...
		},
		{
			"format error",
			func(ctx context.Context, s *svc.ExtrasService, w *bytes.Buffer, m *dbmocks.MockDatasetRepo) error {
				m.EXPECT().ExtrasFormatRows(mock.Anything, "T20").Return(nil, errors.New("boom"))
				return s.ExportFormat(ctx, "T20", w)
			},
			assertErrContains("boom"),
		},
	}
...
func TestExtrasService_WriterError(t *testing.T) {
	t.Parallel()
	m := dbmocks.NewMockDatasetRepo(t)
	s := svc.NewExtrasService(m)
	m.EXPECT().ExtrasUnifiedRows(mock.Anything).Return([][]string{{"h1"}, {"v"}}, nil)
