...
func (f *fakeRepoB) BattingUnifiedRows(context.Context) ([][]string, error) { return f.batUnified, f.batErr }
func (f *fakeRepoB) BattingLegacyRows(context.Context) ([][]string, error) { return f.batLegacy, f.batErr }
...
func assertNoErrorCSVB(want string) assertFnB {
	return func(t *testing.T, w *bytes.Buffer, err error) {
		...
		if w.String() != want { t.Fatalf("csv mismatch\nwant:\n%s\n---\ngot:\n%s", want, w.String()) }
	}
}
...
func assertErrContainsB(sub string) assertFnB {
	return func(t *testing.T, _ *bytes.Buffer, err error) {
		...
		if err == nil || indexOfB(s, sub) < 0 { t.Fatalf("want err containing %q, got %v", sub, err) }
	}
}
...
func TestBowlingService_Exports(t *testing.T) {
	...
	cases := []struct{
		...
		{"unified writes rows", func(ctx context.Context, s *svc.BowlingService, w *bytes.Buffer) error { return s.ExportUnified(ctx, w) }, "h1,h2\n1,2\n", assertNoErrorCSVB("h1,h2\n1,2\n")},
		{"legacy writes rows", func(ctx context.Context, s *svc.BowlingService, w *bytes.Buffer) error { return s.ExportLegacy(ctx, w) }, "lh1,lh2\n3,4\n", assertNoErrorCSVB("lh1,lh2\n3,4\n")},
		{"inference writes rows", func(ctx context.Context, s *svc.BowlingService, w *bytes.Buffer) error { return s.ExportInference(ctx, "T20I", w) }, "ih1,ih2\n5,6\n", assertNoErrorCSVB("ih1,ih2\n5,6\n")},
	}
...
func TestBowlingService_Errors(t *testing.T) {
	...
	cases := []struct{
		...
		{"unified error", func(ctx context.Context, s *svc.BowlingService, w *bytes.Buffer) error { return s.ExportUnified(ctx, w) }, assertErrContainsB("fail")},
		{"legacy error", func(ctx context.Context, s *svc.BowlingService, w *bytes.Buffer) error { return s.ExportLegacy(ctx, w) }, assertErrContainsB("fail")},
		{"inference error", func(ctx context.Context, s *svc.BowlingService, w *bytes.Buffer) error { return s.ExportInference(ctx, "T20I", w) }, assertErrContainsB("fail")},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			buf := &bytes.Buffer{}
...
func TestBowlingService_WriterError(t *testing.T) {
	...
	cases := []struct{
		...
		{"write fails", func(ctx context.Context, s *svc.BowlingService, w io.Writer) error { return s.ExportUnified(ctx, w) }, assertErrContainsB("sink write error")},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
