...
func (f *fakeLoader) Load(_ context.Context, _ string, id string) ([]byte, error) {
	f.mu.Lock(); f.seen = append(f.seen, id); f.mu.Unlock()
	if f.err != nil { return nil, f.err }
	b, ok := f.load[id]
	if !ok { return nil, fmt.Errorf("missing: %s", id) }
	return b, nil
}
...
func assertNoErrorProcessed(want int) assertSvcFn {
	return func(t *testing.T, processed int, err error, _ *fakeLoader, _ *fakeRepo) {
		...
		if processed != want { t.Fatalf("want processed=%d got=%d", want, processed) }
	}
}
...
func assertRepoBatches(want int) assertSvcFn {
	return func(t *testing.T, _ int, err error, _ *fakeLoader, r *fakeRepo) {
		if err != nil { t.Fatalf("unexpected err: %v", err) }
		if len(r.upserts) != want { t.Fatalf("want %d upsert batches, got %d", want, len(r.upserts)) }
	}
}
...
func assertLoaderSaw(ids ...string) assertSvcFn {
	return func(t *testing.T, _ int, err error, fl *fakeLoader, _ *fakeRepo) {
		...
		sort.Strings(got)
		sort.Strings(ids)
		if len(got) != len(ids) { t.Fatalf("loader saw %v, want %v", got, ids) }
		for i := range ids { if got[i] != ids[i] { t.Fatalf("loader saw %v, want %v", got, ids) } }
	}
}
...
func TestIngestService_BasicFlows(t *testing.T) {
	...
	cases := []struct{
		...
		{"dry-run single worker", false, 1, func(t *testing.T, p int, e error, fl *fakeLoader, fr *fakeRepo){ assertNoErrorProcessed(2)(t,p,e,fl,fr); assertRepoBatches(0)(t,p,e,fl,fr); assertLoaderSaw("a.json","b.json")(t,p,e,fl,fr) }},
		{"apply with 2 workers", true, 2, func(t *testing.T, p int, e error, fl *fakeLoader, fr *fakeRepo){ assertNoErrorProcessed(2)(t,p,e,fl,fr); assertRepoBatches(2)(t,p,e,fl,fr) }},
	}
...
func TestIngestService_Errors(t *testing.T) {
		...
		cases := []struct{
			...
			{"nil deps", func() (*svc.IngestService, *fakeLoader, *fakeRepo) { return &svc.IngestService{}, nil, nil }, ".", assertErrContains("nil service")},
			{"empty dir", func() (*svc.IngestService, *fakeLoader, *fakeRepo) { s, fl, fr := mk(); return s, fl, fr }, "", assertErrContains("input directory")},
			{"list error", func() (*svc.IngestService, *fakeLoader, *fakeRepo) { s, fl, fr := mk(); fl.err = errors.New("boom"); return s, fl, fr }, ".", assertErrContains("boom")},
			{"load error", func() (*svc.IngestService, *fakeLoader, *fakeRepo) { s, fl, fr := mk(); fl.err = nil; s.Loader = &fakeLoader{list: []string{"y.json"}, load: map[string][]byte{}, err: nil}; return s, s.Loader.(*fakeLoader), fr }, ".", assertErrContains("missing")},
			{"parse error", func() (*svc.IngestService, *fakeLoader, *fakeRepo) { s, fl, fr := mk(); s.Parser = &fakeParser{err: errors.New("parse")}; return s, fl, fr }, ".", assertErrContains("parse")},
			{"repo error", func() (*svc.IngestService, *fakeLoader, *fakeRepo) { s, fl, fr := mk(); fr.err = errors.New("db"); return s, fl, fr }, ".", assertErrContains("db")},
		}
		for _, tc := range cases {
			t.Run(tc.name, func(t *testing.T) {
				s, fl, fr := tc.arr()
