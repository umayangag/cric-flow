...
func TestLoadFromCSV_EmptyFloatsAndBoolVariants(t *testing.T) {
	...
	if len(ps) != 3 { t.Fatalf("want 3 players got %d", len(ps)) }
	// First row empty floats become 0
	if ps[0].BatScore != 0 || ps[0].BowlScore != 0 || !ps[0].IsBowler || ps[0].IsKeeper { t.Fatalf("unexpected row A: %#v", ps[0]) }
 // Second row space-only bat score becomes 0 after trim; is_keeper=1 recognized
	if ps[1].BatScore != 0 || ps[1].BowlScore != 0.4 || !ps[1].IsBowler || !ps[1].IsKeeper { t.Fatalf("unexpected row B: %#v", ps[1]) }
	// Third row keeper YES recognized
	if !ps[2].IsKeeper { t.Fatalf("expected C to be keeper") }
}
