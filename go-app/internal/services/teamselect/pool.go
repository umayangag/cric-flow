...
func LoadFromDB(ctx context.Context, repo db.TeamSelectRepo, matchID int64, format, season string) ([]Player, error) {
	if repo == nil { return nil, errors.New("nil repo") }
...
func normalizeHeader(h []string) []string {
	...
	for i, v := range h { out[i] = strings.ToLower(strings.TrimSpace(v)) }
	return out
}

func hasHeader(h []string, need []string) bool {
	for _, n := range need { if idx(h, n) < 0 { return false } }
	return true
}
...
func fieldErr(i int, col string) error { return errors.New("row " + strconv.Itoa(i) + ": invalid " + col) }
