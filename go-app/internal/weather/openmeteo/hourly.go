...
func (c *Client) Hourly(ctx context.Context, lat, lon float64, from, to time.Time, tz string) ([]HourlyRecord, error) {
	...
	params.Set(
		"hourly",
		"temperature_2m,apparent_temperature,relative_humidity_2m,precipitation,cloud_cover,pressure_msl,wind_speed_10m,wind_gusts_10m",
	)
	...
	for attempt := 1; attempt <= c.MaxRetry; attempt++ {
		...
		func() {
			...
			if resp.StatusCode != 200 {
				lastErr = fmt.Errorf("hourly status: %d", resp.StatusCode)
				return
			}
			...
			for i := 0; i < n; i++ {
				...
				if i < len(hr.Hourly.Temperature2m) {
					v := int(hr.Hourly.Temperature2m[i] + 0.5)
					t = &v
				}
				if i < len(hr.Hourly.ApparentTemperature) {
					v := int(hr.Hourly.ApparentTemperature[i] + 0.5)
					feels = &v
				}
				if i < len(hr.Hourly.WindSpeed10m) {
					v := int(hr.Hourly.WindSpeed10m[i] + 0.5)
					wind = &v
				}
				if i < len(hr.Hourly.WindGusts10m) {
					v := int(hr.Hourly.WindGusts10m[i] + 0.5)
					gust = &v
				}
				if i < len(hr.Hourly.Precipitation) {
					v := int(hr.Hourly.Precipitation[i] + 0.5)
					rain = &v
				}
				if i < len(hr.Hourly.RelativeHumidity2m) {
					v := hr.Hourly.RelativeHumidity2m[i]
					hum = &v
				}
				if i < len(hr.Hourly.CloudCover) {
					v := hr.Hourly.CloudCover[i]
					cloud = &v
				}
				if i < len(hr.Hourly.PressureMsl) {
					v := int(hr.Hourly.PressureMsl[i] + 0.5)
					press = &v
				}
				out = append(
					out,
