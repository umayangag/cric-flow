...
func getNextSeasonHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if cutoffStr == "" {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "missing cutoff date", Hint: "provide cutoff=YYYY-MM-DD"})
        return
    }
    cutoff, err := time.Parse("2006-01-02", cutoffStr)
    if err != nil {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "invalid cutoff date", Hint: "expected format YYYY-MM-DD"})
        return
    }
