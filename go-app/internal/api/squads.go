...
func getMatchSquadsHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if idStr == "" {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "missing id", Hint: "provide path /match/{id}/squads with numeric id"})
        return
    }
    matchID, err := strconv.ParseInt(idStr, 10, 64)
    if err != nil || matchID <= 0 {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "invalid id", Hint: "id must be a positive integer"})
        return
    }
    ...
    asofStr := q.Get("asof")
    if asofStr == "" {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "missing asof date", Hint: "provide asof=YYYY-MM-DD"})
        return
    }
    asof, err := time.Parse("2006-01-02", asofStr)
    if err != nil {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "invalid asof date", Hint: "expected format YYYY-MM-DD"})
        return
    }
    format := q.Get("format")
    ...
    if getMatchSquadsFunc == nil {
        ...
        writeJSON(w, http.StatusNotImplemented, apiError{Code: "NOT_IMPLEMENTED", Message: "squads retrieval not implemented yet"})
        return
    }
    ...
    if err != nil {
        ...
        case errors.Is(err, errIncompleteSquad) || errors.Is(err, db.ErrIncompleteSquads):
            writeJSON(w, http.StatusUnprocessableEntity, apiError{Code: "INCOMPLETE_SQUADS", Message: "one or both squads are incomplete", Hint: "ensure both teams and players exist for the match"})
            return
        default:
            respondErr(w, err)
            return
