...
func listMatchesHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if seasonStr == "" {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "missing season", Hint: "provide season=YYYY"})
        return
    }
    season, err := strconv.Atoi(seasonStr)
    if err != nil || season < 1000 || season > 9999 {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "invalid season", Hint: "expected 4-digit year"})
        return
    }

    if afterStr == "" {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "missing after date", Hint: "provide after=YYYY-MM-DD"})
        return
    }
    after, err := time.Parse("2006-01-02", afterStr)
    if err != nil {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "invalid after date", Hint: "expected format YYYY-MM-DD"})
        return
    }
