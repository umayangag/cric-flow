...
func TestBacktestMatchHandler_EvaluateMode_MatchAggregatesMetrics(t *testing.T) {
    ...
    if payload.MatchAggregates.Predicted == nil || payload.MatchAggregates.Actual == nil || payload.MatchAggregates.Errors == nil {
...
func TestBacktestMatchHandler_EvaluateMode_FeaturesSeamCalled(t *testing.T) {
    ...
    var gotIDs []int64
    getBacktestFeaturesAtCutoffFunc = func(ctx context.Context, cutoffArg time.Time, playerIDs []int64) (map[int64]map[string]float64, error) {
        called = true
        gotCutoff = cutoffArg
