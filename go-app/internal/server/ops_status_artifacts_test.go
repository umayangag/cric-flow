        ...
        case "/artifacts/status":
            _ = json.NewEncoder(w).Encode(map[string]any{
                "formats": map[string]any{
                    "ODI":  map[string]any{"batting": map[string]any{"exists": true, "loaded": true, "path": "/a.joblib"}, "bowling": map[string]any{"exists": false}},
                    "TEST": map[string]any{"batting": map[string]any{"exists": false}, "bowling": map[string]any{"exists": true, "path": "/b.joblib"}},
                    "T20I": map[string]any{"batting": map[string]any{"exists": false}, "bowling": map[string]any{"exists": false}},
                    "T20":  map[string]any{"batting": map[string]any{"exists": false}, "bowling": map[string]any{"exists": false}},
                },
            })
        default:
            http.NotFound(w, r)
    ...
    tests := []struct{
    ...
    }{
        {
            ...
            assert: func(t *testing.T, sec map[string]any, mlOK bool) {
                if !mlOK { t.Fatalf("expected mlOK=true") }
                fm := sec["formats"].(map[string]any)
                // Check values came from HTTP detail payload
                odi := fm["ODI"].(map[string]any)["batting"].(map[string]any)
                if ex := odi["exists"].(bool); !ex { t.Fatalf("ODI batting should exist from detail endpoint") }
                testBowl := fm["TEST"].(map[string]any)["bowling"].(map[string]any)
                if ex := testBowl["exists"].(bool); !ex { t.Fatalf("TEST bowling should exist from detail endpoint") }
            },
        },
        {
            ...
            assert: func(t *testing.T, sec map[string]any, mlOK bool) {
                if mlOK { t.Fatalf("expected mlOK=false") }
                fm := sec["formats"].(map[string]any)
                odi := fm["ODI"].(map[string]any)
                if !odi["batting"].(map[string]any)["exists"].(bool) || !odi["bowling"].(map[string]any)["exists"].(bool) {
        ...
        },
        {
            ...
            assert: func(t *testing.T, sec map[string]any, mlOK bool) {
                ...
                for _, f := range []string{"TEST","ODI","T20I","T20"} {
                    ...
                    if ent["batting"].(map[string]any)["exists"].(bool) || ent["bowling"].(map[string]any)["exists"].(bool) {
