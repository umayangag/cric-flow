...
func TestComputeSuggestions_Table(t *testing.T) {
    ...
    noSuggestions := func() assertion {
        return func(t *testing.T, got []map[string]any) {
            if len(got) != 0 { t.Fatalf("expected no suggestions, got %v", got) }
        }
    }
    ...
    tests := []struct{
        ...
        },
        {
            ...
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 0.0, "matches": 5.0}}, services: map[string]bool{"ml_health": true}},
            assert: hasAll("make migrate", "cricsheet-import"),
        },
        {
            name: "precompute_stale_and_missing",
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 1.0, "matches": 1.0}}, pre: map[string]any{"formats": map[string]any{"ODI": map[string]any{"status": "stale"}, "T20I": map[string]any{"status": "missing"}, "TEST": map[string]any{"status": "ok"}, "T20": map[string]any{"status": "ok"}}}, services: map[string]bool{"ml_health": true}},
            assert: hasAll("Precompute missing/stale for ODI,T20I", "make precompute", "precompute-asof"),
        },
        {
            name: "exports_missing_single_and_multi",
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 1.0, "matches": 1.0}},
        ...
        },
        {
            ...
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 5.0, "matches": 7.0}},
        ...
        },
        {
            ...
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 10.0, "matches": 20.0}},
                pre: map[string]any{"formats": map[string]any{"TEST": map[string]any{"status": "ok"}, "ODI": map[string]any{"status": "ok"}, "T20I": map[string]any{"status": "ok"}, "T20": map[string]any{"status": "ok"}}},
                exp: map[string]any{"formats": map[string]any{"TEST": map[string]any{"files": []map[string]any{{"name": "a.csv", "exists": true}}}, "ODI": map[string]any{"files": []map[string]any{{"name": "b.csv", "exists": true}}}, "T20I": map[string]any{"files": []map[string]any{{"name": "c.csv", "exists": true}}}, "T20": map[string]any{"files": []map[string]any{{"name": "d.csv", "exists": true}}}}},
                arts: map[string]any{"formats": map[string]any{"TEST": map[string]any{"batting": map[string]any{"exists": true}, "bowling": map[string]any{"exists": true}}, "ODI": map[string]any{"batting": map[string]any{"exists": true}, "bowling": map[string]any{"exists": true}}, "T20I": map[string]any{"batting": map[string]any{"exists": true}, "bowling": map[string]any{"exists": true}}, "T20": map[string]any{"batting": map[string]any{"exists": true}, "bowling": map[string]any{"exists": true}}}},
                services: map[string]bool{"ml_health": true}, fielding: map[string]any{"available": true, "rows": 100.0}, weather: map[string]any{"available": true, "rows": 200.0}},
    ...
    for _, tc := range tests {
        t.Run(tc.name, func(t *testing.T) {
            got := computeSuggestions(tc.setup.db, tc.setup.pre, tc.setup.exp, tc.setup.arts, tc.setup.services, tc.setup.fielding, tc.setup.weather)
            tc.assert(t, got)
        })
    }
}
