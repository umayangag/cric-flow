...
func TestComputeSuggestions_Table(t *testing.T) {
    ...
    tests := []struct{
        ...
        },
        {
            ...
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 0.0, "matches": 5.0, "innings": 7.0}}, services: map[string]bool{"ml_health": true}},
            assert: hasAll("make migrate", "cricsheet-import"),
        },
        {
            name: "precompute_stale_and_missing",
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 1.0, "matches": 1.0, "innings": 1.0}}, pre: map[string]any{"formats": map[string]any{"ODI": map[string]any{"status": "stale"}, "T20I": map[string]any{"status": "missing"}, "TEST": map[string]any{"status": "ok"}, "T20": map[string]any{"status": "ok"}}}, services: map[string]bool{"ml_health": true}},
            assert: hasAll("Precompute missing/stale for ODI,T20I", "make precompute", "precompute-asof"),
        },
        {
            name: "exports_missing_single_and_multi",
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 1.0, "matches": 1.0, "innings": 1.0}},
        ...
        },
        {
            ...
            setup: setup{db: map[string]any{"connected": true, "counts": map[string]any{"players": 10.0, "matches": 20.0, "innings": 30.0}},
    ...
    for _, tc := range tests {
        t.Run(tc.name, func(t *testing.T) {
            got := computeSuggestions(tc.setup.db, tc.setup.pre, tc.setup.exp, tc.setup.arts, tc.setup.services)
            tc.assert(t, got)
        })
    }
}
