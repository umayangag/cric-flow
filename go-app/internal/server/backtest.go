...
func init() {
	...
	mlBacktestPredictFunc = func(ctx context.Context, cutoff time.Time, format string, playerIDs []int64, features map[int64]map[string]float64, matchCtx *MatchContextForReconciliation) (map[int64]playerPredictions, error) {
		if mlClient == nil {
			return nil, errors.New("ml client not initialized")
		}
		return mlClient.predictPlayers(ctx, cutoff, format, playerIDs, features, matchCtx)
	}
	...
	mlSimulateMatchXIFunc = func(ctx context.Context, req predictteam.XISimulationRequest) (*predictteam.XISimulationResult, error) {
		if mlClient == nil {
			return nil, errors.New("ml client not initialized")
		}
		return mlClient.SimulateMatchXI(ctx, req)
	}
