...
func (a *App) backtestMatchHandler(w http.ResponseWriter, r *http.Request) {
    ...
    // Evaluate mode
    if matchID == "" {
        writeJSON(w, http.StatusBadRequest, apiError{Code: "INVALID_PARAM", Message: "match_id is required for evaluate mode"})
        return
    }
    ...
    // Collect actuals and predictions for runs to compute RMSE and R² after loop
    runsActuals := make([]float64, 0, len(squad))
    runsPreds := make([]float64, 0, len(squad))
...
func init() {
    ...
    getBacktestSquadPlayerIDsFunc = func(ctx context.Context, matchID int64, _ time.Time, _ string) ([]int64, error) {
        ...
        rows, err := db.Pool.Query(ctx, `SELECT DISTINCT player_id FROM player_match WHERE match_id = $1 ORDER BY player_id ASC`, matchID)
        if err != nil {
            return nil, err
        }
        defer rows.Close()
