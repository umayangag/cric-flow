...
// mlModelStatsProxyHandler proxies GET to the ML service /model-stats for the ML Model Stats tab.
func (a *App) mlModelStatsProxyHandler(w http.ResponseWriter, r *http.Request) {
	base := strings.TrimSpace(os.Getenv("ML_SERVICE_URL"))
	if base == "" {
		base = config.ServerMLBaseURLFallback(config.Load())
	}
	...
	ctx, cancel := context.WithTimeout(r.Context(), time.Duration(config.ServerMLHealthTimeoutSec(cfg))*time.Second)
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, base+"/model-stats", nil)
	if err != nil {
		slog.Warn("ml model-stats proxy: new request failed", slog.Any("err", err))
		respondJSON(w, http.StatusInternalServerError, map[string]string{"status": "error", "error": err.Error()})
		return
	}
	client := getMLHealthClient()
	resp, err := client.Do(req)
	if err != nil {
		slog.Warn("ml model-stats proxy: request failed", slog.Any("err", err))
		respondJSON(w, http.StatusBadGateway, map[string]string{"status": "error", "error": err.Error()})
		return
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		body, _ := io.ReadAll(io.LimitReader(resp.Body, int64(maxBody)))
		slog.Warn(
			"ml model-stats proxy: upstream non-2xx",
			slog.Int("status", resp.StatusCode),
			slog.String("body", string(body)),
		)
		w.Header().Set("Content-Type", "application/json")
	...
	var payload map[string]any
	if err := json.NewDecoder(io.LimitReader(resp.Body, int64(maxBody))).Decode(&payload); err != nil {
		slog.Warn("ml model-stats proxy: decode failed", slog.Any("err", err))
		respondJSON(
			w,
			http.StatusInternalServerError,
			map[string]string{"status": "error", "error": "invalid ml model-stats response"},
		)
		return
	}
