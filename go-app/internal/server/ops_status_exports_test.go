...
func TestBuildExportsSection_Table(t *testing.T) {
    ...
    assertEmptyFormats := func(root string) assertion {
        return func(t *testing.T, sec map[string]any) {
            if sec["root"] != root { t.Fatalf("root mismatch: got %v", sec["root"]) }
            fm, ok := sec["formats"].(map[string]any); if !ok { t.Fatalf("formats wrong type: %T", sec["formats"]) }
            for _, f := range []string{"TEST","ODI","T20I","T20"} {
                vf, ok := fm[f].(map[string]any); if !ok { t.Fatalf("missing format %s", f) }
                if arr, ok := vf["files"].([]map[string]any); ok {
                    if len(arr) != 0 { t.Fatalf("files not empty for %s", f) }
                } else if arrAny, ok := vf["files"].([]any); ok {
                    if len(arrAny) != 0 { t.Fatalf("files not empty(any) for %s", f) }
                } else {
            ...
            for _, f := range []string{"TEST","ODI","T20I","T20"} {
                ...
                for _, e := range files {
                    ...
                    case "batting_on.csv":
                        ...
                        if rows, ok := e["rows"].(int); ok && rows == 0 { t.Fatalf("batting rows should be >0") }
                        if _, ok := e["modified"].(string); !ok { t.Fatalf("modified missing") }
                    case "bowling_on.csv":
                        seenBowl = e["exists"].(bool)
                    }
                }
                if !seenBat || !seenBowl { t.Fatalf("unified not applied for %s", f) }
            }
        }
    }
    ...
    assertPerFormat := func() assertion {
        return func(t *testing.T, sec map[string]any) {
            ...
            for _, e := range fm["ODI"].(map[string]any)["files"].([]map[string]any) {
                if e["name"] == "my_batting_ODI_2020.csv" && e["exists"].(bool) { hasODI = true }
            }
            if !hasODI { t.Fatalf("ODI batting not detected") }
            // T20I has bowling file
            var hasT20I bool
            for _, e := range fm["T20I"].(map[string]any)["files"].([]map[string]any) {
                if e["name"] == "stats_bowling_T20I.csv" && e["exists"].(bool) { hasT20I = true }
            }
            if !hasT20I { t.Fatalf("T20I bowling not detected") }
        }
    }
