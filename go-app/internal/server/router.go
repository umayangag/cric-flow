...
func NewRouter(a *App) http.Handler {
	...
	admin.HandleFunc("/api/options/teams-by-format", optionsHandler.HandleGetTeamsByFormat).Methods(http.MethodGet, http.MethodOptions)
	admin.HandleFunc("/api/options/opponents", optionsHandler.HandleGetOpponents).Methods(http.MethodGet, http.MethodOptions)
