...
func (productionInsightsProbe) CountMatchesSinceByFormat(ctx context.Context, format string, since time.Time) (int64, error) {
    if db.Pool == nil {
    ...
    if dbCompleteness != nil {
        if fmAny, ok := dbCompleteness["formats"].(map[string]any); ok {
            ...
            if len(low) > 0 {
                sort.Strings(low)
                out = append(out, map[string]any{
                    "reason":   fmt.Sprintf("Few or no matches in last 30 days for %s. Consider re-importing or verifying schedule.", strings.Join(low, ", ")),
                    "commands": []string{"make cricsheet-import"},
                })
            }
        }
