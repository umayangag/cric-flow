...
func (memFS) ReadFile(context.Context, string) ([]byte, error) { return nil, errors.New("not implemented") }
func (memFS) WriteFile(context.Context, string, []byte, fs.FileMode) error { return errors.New("not implemented") }
...
func (f *fakeLoader) List(_ context.Context, _ string) ([]string, error) { if f.err != nil { return nil, f.err }; return f.ids, nil }
func (f *fakeLoader) Load(_ context.Context, _ string, id string) ([]byte, error) { if f.err != nil { return nil, f.err }; return f.data[id], nil }
...
func (r *fakeRepo) UpsertMatches(_ context.Context, ms []domain.Match) error { if r.err != nil { return r.err }; r.got = append([]domain.Match(nil), ms...); return nil }
...
func assertNoErrorUpsertCount(n int) assertFn {
	return func(t *testing.T, repo *fakeRepo, err error) {
		...
		if len(repo.got) != n { t.Fatalf("want upsert %d matches, got %d", n, len(repo.got)) }
	}
}
...
func TestRunner_Run_BasicFlows(t *testing.T) {
 ...
 parser := &fakeParser{matches: map[string][]domain.Match{"a": []domain.Match{domain.Match{ID:1,Format:"T20"}}, "b": []domain.Match{domain.Match{ID:2,Format:"ODI"}}}}
	repo := &fakeRepo{}
...
func TestRunner_Run_DryRun(t *testing.T) {
	...
	parser := &fakeParser{matches: map[string][]domain.Match{"x": []domain.Match{domain.Match{ID:9,Format:"TEST"}}}}
	repo := &fakeRepo{}
	r := cmd.NewRunner(memFS{}, loader, parser, repo, nil)
...
func TestRunner_Run_Errors(t *testing.T) {
	...
	cases := []struct{
	...
	}{
		{
			...
			arrange: func() (*cmd.Runner, cli.Options, *fakeRepo) { return cmd.NewRunner(memFS{}, &fakeLoader{}, &fakeParser{}, &fakeRepo{}, nil), cli.Options{}, &fakeRepo{} },
			assert: assertErrContains("input directory"),
		},
		{
			name: "list error",
			arrange: func() (*cmd.Runner, cli.Options, *fakeRepo) { fl := &fakeLoader{err: errors.New("boom")}; return cmd.NewRunner(memFS{}, fl, &fakeParser{}, &fakeRepo{}, nil), cli.Options{InDir:"/tmp", Apply:true, Concurrency:1}, &fakeRepo{} },
		...
		},
		{
			...
			arrange: func() (*cmd.Runner, cli.Options, *fakeRepo) {
				...
				fr := &fakeRepo{}
				return cmd.NewRunner(memFS{}, fl, fp, fr, nil), cli.Options{InDir:"/tmp", Apply:true, Concurrency:1}, fr
			},
