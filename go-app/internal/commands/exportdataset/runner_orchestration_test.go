...
func (m *memFS) WriteFile(_ context.Context, path string, data []byte, perm fs.FileMode) error {
	if m.writes == nil { m.writes = map[string][]byte{} }
	if m.writePerm == nil { m.writePerm = map[string]fs.FileMode{} }
	if m.writeErr != nil { return m.writeErr }
...
func assertNoErrorUnifiedOrch(outDir string) assertOrchFn {
	return func(t *testing.T, fs *memFS, bat *fakeBat, bow *fakeBow, err error) {
		...
		if bat.unified != 1 || bow.unified != 1 { t.Fatalf("want unified calls bat=1 bow=1, got %d %d", bat.unified, bow.unified) }
