...
func assertNoErrorOpts(want cli.Options) assertFn {
	return func(t *testing.T, got cli.Options, err error) {
		...
		if got.Size != want.Size { t.Fatalf("want Size=%d got %d", want.Size, got.Size) }
		if got.MinBowlers != want.MinBowlers { t.Fatalf("want MinBowlers=%d got %d", want.MinBowlers, got.MinBowlers) }
		if got.RequireKeeper != want.RequireKeeper { t.Fatalf("want RequireKeeper=%v got %v", want.RequireKeeper, got.RequireKeeper) }
		if got.FromDB != want.FromDB { t.Fatalf("want FromDB=%v got %v", want.FromDB, got.FromDB) }
		if got.PoolCSV != want.PoolCSV { t.Fatalf("want PoolCSV=%q got %q", want.PoolCSV, got.PoolCSV) }
	}
}
...
func TestParseArgs_Basic(t *testing.T) {
	...
	cases := []struct{
	...
	}{
		{
			...
			args: []string{"-match","1193505","-format","T20","-season","2019","-size","11","-min-bowlers","5","-require-keeper"},
			assert: assertNoErrorOpts(cli.Options{MatchID:1193505,Format:"T20",Season:"2019",Size:11,MinBowlers:5,RequireKeeper:true,FromDB:true}),
		},
		{
			name: "happy path from CSV",
			setup: func(){},
			args: []string{"-match","1193505","-format","ODI","-season","2019","-size","11","-min-bowlers","4","-from-db=false","-pool","/tmp/pool.csv"},
			assert: assertNoErrorOpts(cli.Options{MatchID:1193505,Format:"ODI",Season:"2019",Size:11,MinBowlers:4,FromDB:false,PoolCSV:"/tmp/pool.csv"}),
		},
		{
			name: "invalid match",
			setup: func(){},
