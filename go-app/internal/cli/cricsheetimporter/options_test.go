...
func assertNoErrorApplyDir(wantDir string, wantApply bool, wantConc int) assertFn {
	return func(t *testing.T, got cli.Options, err error) {
		...
		if got.InDir != wantDir { t.Fatalf("want InDir=%q got %q", wantDir, got.InDir) }
		if got.Apply != wantApply { t.Fatalf("want Apply=%v got %v", wantApply, got.Apply) }
		if got.Concurrency != wantConc { t.Fatalf("want Concurrency=%d got %d", wantConc, got.Concurrency) }
	}
}
...
func TestParseArgs_Basic(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			os.Unsetenv("GO_APP_CRICSHEET_DIR"); os.Unsetenv("CRICSHEET_CONCURRENCY")
