...
func assertNoErrorOpts(match int64, provider string, apply bool) assertFn {
	return func(t *testing.T, got cli.Options, err error) {
		...
		if got.MatchID != match {
			t.Fatalf("want MatchID=%d got %d", match, got.MatchID)
		}
		if got.Provider != provider {
			t.Fatalf("want Provider=%q got %q", provider, got.Provider)
		}
		if got.Apply != apply {
			t.Fatalf("want Apply=%v got %v", apply, got.Apply)
		}
	}
}
