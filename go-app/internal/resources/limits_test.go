...
func TestMemoryBasedLimit(t *testing.T) {
	tests := []struct {
		...
		// 2GiB * 100% / per-worker MB: precompute uses default 450 → 2048/450≈4; seqcalc capped at 1 for ≤2GB
		{"precompute 2GiB", KindPrecompute, 2 * 1024 * 1024 * 1024, 4},
		{"import 2GiB", KindImport, 2 * 1024 * 1024 * 1024, 13},
		{"seqcalc 2GiB", KindSeqCalc, 2 * 1024 * 1024 * 1024, 1},
		// 512MiB * 100% / 450 MiB < 1 → 1
