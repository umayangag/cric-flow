...
func TestMemoryBasedLimit(t *testing.T) {
	tests := []struct {
		...
		// 2GiB * 80% / per-worker MB: precompute 1638/450≈3, import 1638/150≈10, seqcalc 1638/500≈3
		{"precompute 2GiB", KindPrecompute, 2 * 1024 * 1024 * 1024, 3},
		{"import 2GiB", KindImport, 2 * 1024 * 1024 * 1024, 10},
		{"seqcalc 2GiB", KindSeqCalc, 2 * 1024 * 1024 * 1024, 3},
		// 512MiB * 80% / 450 MiB < 1 → 1
