...
We prefer interface-driven code with mocks generated by `mockery`. A minimal subset is configured in `.mockery.yaml`. If your local `mockery` tool is not installed or encounters loader panics on some packages, you can either install/pin a stable version or temporarily skip mock generation:

```
# Install mockery (macOS)
brew install mockery
# Linux/CI: install via your package manager or fallback:
#   go install github.com/vektra/mockery/v2@latest

# Generate mocks (best-effort)
make -C go-app mocks || true
```
