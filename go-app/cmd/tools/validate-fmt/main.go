...
func main() {
	...
	flag.StringVar(&formatsCSV, "formats", "", "comma-separated list of format codes to validate (e.g., ODI,T20I). Empty = all")
	flag.Parse()
	...
	type key struct{ format string; season int64 }
	...
	for _, code := range formatCodes {
		...
		rows, err := db.Pool.Query(ctx, `SELECT COALESCE(season_id, 0) AS season_id, COUNT(*) FROM match_details WHERE format_id=$1 GROUP BY season_id`, formatID)
		if err != nil { log.Fatalf("query match_details: %v", err) }
		for rows.Next() {
			var sid int64; var cnt int64
			if err := rows.Scan(&sid, &cnt); err != nil { log.Fatalf("scan md: %v", err) }
			matchCounts[key{code, sid}] = cnt
		}
		rows.Close()
		// Feature table counts (distinct keys)
		rows2, err := db.Pool.Query(ctx, `SELECT season_id, COUNT(*) FROM player_form_data_fmt WHERE format_id=$1 GROUP BY season_id`, formatID)
		if err == nil {
			for rows2.Next() { var sid int64; var cnt int64; _ = rows2.Scan(&sid, &cnt); formCounts[key{code, sid}] = cnt }
			rows2.Close()
		}
		rows3, err := db.Pool.Query(ctx, `SELECT 0 as season_id, COUNT(*) FROM player_venue_data_fmt WHERE format_id=$1`, formatID)
		if err == nil { for rows3.Next() { var sid int64; var cnt int64; _ = rows3.Scan(&sid, &cnt); venueCounts[key{code, sid}] = cnt } ; rows3.Close() }
		rows4, err := db.Pool.Query(ctx, `SELECT 0 as season_id, COUNT(*) FROM player_opposition_data_fmt WHERE format_id=$1`, formatID)
		if err == nil { for rows4.Next() { var sid int64; var cnt int64; _ = rows4.Scan(&sid, &cnt); oppoCounts[key{code, sid}] = cnt } ; rows4.Close() }
	}
	...
	// Collect all keys
	all := map[key]struct{}{}
	for k := range matchCounts { all[k] = struct{}{} }
	for k := range formCounts { all[k] = struct{}{} }
	// Include venue/oppo with season 0
	for k := range venueCounts { all[k] = struct{}{} }
	for k := range oppoCounts { all[k] = struct{}{} }
	keys := make([]key, 0, len(all))
	for k := range all { keys = append(keys, k) }
	sort.Slice(keys, func(i, j int) bool {
		if keys[i].format == keys[j].format { return keys[i].season < keys[j].season }
	...
	if bad > 0 {
		fmt.Fprintf(os.Stderr, "validation failed: %d gaps found (formats with matches but empty feature tables)\n", bad)
		os.Exit(1)
	}
