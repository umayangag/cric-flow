...
func importWeather(ctx context.Context, path string, defaultSession string) {
	fh, err := os.Open(path)
	if err != nil {
		if os.IsNotExist(err) { log.Printf("skip weather: %s not found", path); return }
		log.Printf("open %s: %v", path, err); return
	}
	defer fh.Close()
	...
	r.FieldsPerRecord = -1
	head, err := r.Read()
	if err != nil { log.Printf("read header %s: %v", path, err); return }
	idx := makeIndex(head)
	rows, err := r.ReadAll()
	if err != nil { log.Printf("read rows %s: %v", path, err); return }
	var count int
	for _, rec := range rows {
		get := func(name string) string { if i, ok := idx[strings.ToLower(name)]; ok && i < len(rec) { return strings.TrimSpace(rec[i]) } ; return "" }
		matchID := atoi64(get("match_id"))
		if matchID == 0 { continue }
		session := get("session")
		if session == "" { session = defaultSession }
		w := &db.Weather{ MatchID: matchID, Session: session,
			Temp: atoiPtr(get("temp")), Feels: atoiPtr(get("feels")), Wind: atoiPtr(get("wind")), Gust: atoiPtr(get("gust")),
			Rain: atoiPtr(get("rain")), Humidity: atoiPtr(get("humidity")), Cloud: atoiPtr(get("cloud")), Pressure: atoiPtr(get("pressure")),
		}
		if v := get("viscosity"); v != "" { w.Viscosity = &[]string{v}[0] }
...
func importBatting(ctx context.Context, path string) {
	fh, err := os.Open(path)
	if err != nil { if os.IsNotExist(err) { log.Printf("skip batting: %s not found", path); return } ; log.Printf("open %s: %v", path, err); return }
	defer fh.Close()
	...
	for _, rec := range rows {
		...
		if matchID == 0 || playerName == "" { continue }
		playerID, err := db.GetOrCreateByName(ctx, playerName)
		if err != nil { log.Printf("player %q: %v", playerName, err); continue }
		...
		b := &db.Batting{ MatchID: matchID, PlayerID: playerID, Description: strPtrOrNil(desc), Runs: runs, Balls: balls, Minutes: mins, Fours: fours, Sixes: sixes, StrikeRate: sr, BattingPosition: pos }
		if err := db.UpsertBatting(ctx, b); err != nil { log.Printf("upsert batting mid=%d pid=%d: %v", matchID, playerID, err); continue }
		count++
	}
...
func importBowling(ctx context.Context, path string) {
	fh, err := os.Open(path)
	if err != nil { if os.IsNotExist(err) { log.Printf("skip bowling: %s not found", path); return } ; log.Printf("open %s: %v", path, err); return }
	defer fh.Close()
	...
	for _, rec := range rows {
		...
		b := &db.Bowling{ MatchID: matchID, PlayerID: playerID, Overs: ov, Balls: bl, Maidens: md, Runs: rn, Wickets: wk, Dots: dt, Fours: fr, Sixes: sx, Econ: ec, Wides: wd, NoBalls: nb }
		if err := db.UpsertBowling(ctx, b); err != nil { log.Printf("upsert bowling mid=%d pid=%d: %v", matchID, playerID, err); continue }
		count++
	}
...
func importFielding(ctx context.Context, path string) {
	fh, err := os.Open(path)
	if err != nil { if os.IsNotExist(err) { return } ; log.Printf("open %s: %v", path, err); return }
	defer fh.Close()
	...
	for _, rec := range rows {
		...
		f := &db.Fielding{ MatchID: matchID, PlayerID: playerID, Catches: ca, RunOuts: ro, DroppedCatches: dc, MissedRunOuts: mr }
		if err := db.UpsertFielding(ctx, f); err != nil { log.Printf("upsert fielding mid=%d pid=%d: %v", matchID, playerID, err); continue }
		count++
	}
...
func makeIndex(head []string) map[string]int {
	idx := map[string]int{}
	for i, h := range head { idx[strings.ToLower(strings.TrimSpace(h))] = i }
	return idx
}

func atoi64(s string) int64 { v, _ := strconv.ParseInt(strings.TrimSpace(s), 10, 64); return v }
func atoiPtr(s string) *int { if s == "" { return nil }; v, err := strconv.Atoi(strings.TrimSpace(s)); if err != nil { return nil }; return &v }
func atof32Ptr(s string) *float32 { if s == "" { return nil }; f, err := strconv.ParseFloat(strings.TrimSpace(s), 32); if err != nil { return nil }; v := float32(f); return &v }
