"""Tests for backtest cache helpers and prediction_settings datetime rounding."""

from datetime import datetime, timezone

from app.main import get_backtest_compute_counts, reset_backtest_cache
from app.prediction_settings import round_datetime_to_granularity
...
def test_round_datetime_to_granularity_none_returns_unchanged():
    ...
    assert round_datetime_to_granularity(dt, "none") == dt
    assert round_datetime_to_granularity(dt, "") == dt
    assert round_datetime_to_granularity(dt, "NONE") == dt
...
def test_round_datetime_to_granularity_second():
    ...
    dt = datetime(2025, 2, 26, 14, 32, 11, 123456, tzinfo=timezone.utc)
    got = round_datetime_to_granularity(dt, "second")
...
def test_round_datetime_to_granularity_minute():
    ...
    dt = datetime(2025, 2, 26, 14, 32, 11, 123456, tzinfo=timezone.utc)
    got = round_datetime_to_granularity(dt, "minute")
...
def test_round_datetime_to_granularity_hour():
    ...
    dt = datetime(2025, 2, 26, 14, 32, 11, 123456, tzinfo=timezone.utc)
    got = round_datetime_to_granularity(dt, "hour")
...
def test_round_datetime_to_granularity_day():
    ...
    dt = datetime(2025, 2, 26, 14, 32, 11, 123456, tzinfo=timezone.utc)
    got = round_datetime_to_granularity(dt, "day")
...
def test_round_datetime_to_granularity_unknown_fallback():
    ...
    dt = datetime(2025, 2, 26, 14, 32, 11, 123456, tzinfo=timezone.utc)
    assert round_datetime_to_granularity(dt, "invalid") == dt
