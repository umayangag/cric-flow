...
def train_a_run():
    ...
    tp.start("batting", total_formats=2)
    tp.data_loaded("batting", "T20I", rows=1200, features=42, targets=5)
    tp.columns_dropped("batting", "T20I", ["weather_composite"], kept=41)
    tp.fitting("batting", "T20I", 1200, 41)
    tp.format_done("batting", "T20I", {"rmse": 24.1})
    tp.data_loaded("batting", "ODI", rows=900, features=42, targets=5)
    tp.format_done("batting", "ODI", {"rmse": 31.7})
...
def test_the_summary_outlives_the_progress_file():
    ...
    assert training_orchestrator.get_step_progress("train_batting") == {}, "progress is gone, as it should be"
    summary = training_orchestrator.get_run_summary("train_batting")
...
def test_the_summary_records_artifacts(tmp_path):
    ...
    tp.start("batting", total_formats=1)
...
def test_a_second_run_does_not_inherit_the_first():
    ...
    tp.start("batting", total_formats=1)
    tp.data_loaded("batting", "TEST", rows=10, features=5)
    tp.format_done("batting", "TEST")
    tp.finish("batting", saved=1)
...
def test_a_run_that_never_finished_has_no_summary():
    ...
    assert training_orchestrator.get_run_summary("train_batting") == {}
...
def test_summary_does_not_confuse_steps():
    ...
    tp.start("win", total_formats=1)
    tp.format_done("win", "ODI")
    tp.finish("win", saved=1)

    assert training_orchestrator.get_run_summary("train_batting")["formats"][0]["format"] == "T20I"
    assert training_orchestrator.get_run_summary("train_win")["formats"][0]["format"] == "ODI"
...
def test_get_run_summary_by_run_id(monkeypatch):
    ...
    assert training_orchestrator.get_run_summary("train_batting", "named-run")["saved"] == 2
    assert training_orchestrator.get_run_summary("train_batting", "other-run") == {}
...
def test_a_result_file_is_never_mistaken_for_live_progress():
    ...
    assert run_progress.list_for_step("train_batting") == []


def test_train_response_carries_the_summary():
    train_a_run()

    body = training_orchestrator.train_response("batting")
    assert body["status"] == "ok"
    assert body["step"] == "batting"
...
def test_train_response_omits_an_absent_summary():
    ...
    body = training_orchestrator.train_response("combination_meta")

    assert body == {"status": "ok", "step": "combination_meta"}
