...
def get_match_data(match_id, session):
    db_cursor.execute(
        (
            f"SELECT inning, {session}_session, toss, venue_id, opposition_id, season_id, "
            f"score, wickets, balls, target, extras, match_number, result "
...
def get_player_metric(match_id, type, player, metric, metric_type, metric_id):
    if metric_id is not None:
        if metric == "form":
            db_cursor.execute(
                (
                    f"SELECT {type}_form FROM player_form_data_fmt where player_id={player[0]} "
                    f"and season_id={metric_id} and format_id={player[1]}"
                )
            )
        elif metric == "venue":
            db_cursor.execute(
                (
                    f"SELECT {type}_venue FROM player_venue_data_fmt "
                    f"where player_id={player[0]} and venue_id={metric_id} "
                    f"and format_id={player[1]}"
                )
            )
        elif metric == "opposition":
            db_cursor.execute(
                (
                    f"SELECT {type}_opposition FROM player_opposition_data_fmt where player_id={player[0]} "
                    f"and opposition_id={metric_id} and format_id={player[1]}"
                )
            )
