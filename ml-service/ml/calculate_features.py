                ...
                if inning_count > 5:
                    ...
                    db_cursor.execute(
                        (
                            ...
                            "player_id, venue_id, format_id, "
                            "batting_venue, bowling_venue"  # noqa: E501
                            ") VALUES (%s, %s, %s, %s, 0) "
                            "ON CONFLICT (player_id, venue_id, format_id) DO UPDATE SET batting_venue = %s"  # noqa: E501
                        ),
            ...
            for player in players_list:
                ...
                db_cursor.execute(
                    (
                        ...
                        "bd.sixes, bd.strike_rate, "
                        "md.opposition_id, "
                        "md.format_id FROM batting_data bd "
                        "LEFT JOIN match_details md ON bd.match_id = md.match_id "
                        "WHERE bd.player_id = %s AND md.opposition_id = %s AND md.format_id = %s"
                    ),
