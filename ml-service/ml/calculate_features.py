        ...
        for format in format_list:
            for player in players_list:
                ...
                if inning_count > 5:
                    ...
                    db_cursor.execute('INSERT INTO player_form_data_fmt (player_id, season_id, format_id, batting_form, bowling_form) VALUES (%s, %s, %s, %s, 0) ON CONFLICT (player_id, season_id, format_id) DO UPDATE SET batting_form = %s', (player[0], season[0], format[0], form, form))
                ...
                if inning_count > 5:
                    ...
                    db_cursor.execute('UPDATE player_form_data_fmt SET bowling_form = %s WHERE player_id = %s AND season_id = %s AND format_id = %s', (form, player[0], season[0], format[0]))
        ...
        for format in format_list:
            for player in players_list:
                ...
                if inning_count > 5:
                    ...
                    db_cursor.execute('INSERT INTO player_venue_data_fmt (player_id, venue_id, format_id, batting_venue, bowling_venue) VALUES (%s, %s, %s, %s, 0) ON CONFLICT (player_id, venue_id, format_id) DO UPDATE SET batting_venue = %s', (player[0], venue[0], format[0], venue_performance, venue_performance))
                ...
                if inning_count > 5:
                    ...
                    db_cursor.execute('UPDATE player_venue_data_fmt SET bowling_venue = %s WHERE player_id = %s AND venue_id = %s AND format_id = %s', (venue_performance, player[0], venue[0], format[0]))
        ...
        for format in format_list:
            for player in players_list:
                ...
                if inning_count > 5:
                    ...
                    db_cursor.execute('INSERT INTO player_opposition_data_fmt (player_id, opposition_id, format_id, batting_opposition, bowling_opposition) VALUES (%s, %s, %s, %s, 0) ON CONFLICT (player_id, opposition_id, format_id) DO UPDATE SET batting_opposition = %s', (player[0], opposition[0], format[0], opposition_performance, opposition_performance))
                ...
                if inning_count > 5:
                    ...
                    db_cursor.execute('UPDATE player_opposition_data_fmt SET bowling_opposition = %s WHERE player_id = %s AND opposition_id = %s AND format_id = %s', (opposition_performance, player[0], opposition[0], format[0]))
        ...
        for format in format_list:
            for player in players_list:
                ...
                if inning_count > 10:
                    ...
                    db_cursor.execute('INSERT INTO player_consistency_data_fmt (player_id, season_id, format_id, batting_consistency, bowling_consistency) VALUES (%s, %s, %s, %s, 0) ON CONFLICT (player_id, season_id, format_id) DO UPDATE SET batting_consistency = %s', (player[0], season[0], format[0], consistency, consistency))
                ...
                if inning_count > 10:
                    ...
                    db_cursor.execute('UPDATE player_consistency_data_fmt SET bowling_consistency = %s WHERE player_id = %s AND season_id = %s AND format_id = %s', (consistency, player[0], season[0], format[0]))
