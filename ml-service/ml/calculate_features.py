...
def calculate_features(db_connection):
        ...
        for format in format_list:
            for player in players_list:
                ...
                if inning_count > 5:
                    ...
                    print(
                        f"Batting form for {player[1]} in season {season[1]} and format {format[1]}: {form}"
                    )
                    db_cursor.execute(
                ...
                if inning_count > 5:
                    ...
                    print(
                        f"Bowling form for {player[1]} in season {season[1]} and format {format[1]}: {form}"
                    )
                    db_cursor.execute(
    ...
    for player in players_list:
        ...
        if inning_count > 10:
            ...
            db_cursor.execute(
                f"INSERT INTO player_consistency_data_fmt (player_id, season_id, format_id, batting_consistency, bowling_consistency) VALUES ({player[0]}, {season[0]}, {format[0]}, {consistency}, 0) ON CONFLICT (player_id, season_id, format_id) DO UPDATE SET batting_consistency = {consistency}"
            )
    ...
    for player in players_list:
        ...
        if inning_count > 10:
            ...
            db_cursor.execute(
                f"UPDATE player_consistency_data_fmt SET bowling_consistency = {consistency} WHERE player_id = {player[0]} AND season_id = {season[0]} AND format_id = {format[0]}"
            )
