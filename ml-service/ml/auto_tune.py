"""
Auto-tune hyperparameters and model type for batting, bowling, and fielding regressors.
...
same format as train_batting/train_bowling/train_fielding, plus a tuning report JSON.

Usage:
  # From CSV (per-format)
  python -m ml.auto_tune --model batting --csv ../output/go-app/batting_encoded_T20.csv --format T20
  python -m ml.auto_tune --model bowling --csv ../output/go-app/bowling_encoded_T20.csv --format T20
  python -m ml.auto_tune --model fielding --csv path/to/fielding.csv --format T20

  # From go-app API (requires GO_APP_URL and --cutoff)
  GO_APP_URL=http://localhost:8080 python -m ml.auto_tune --model batting --from-api --cutoff 2024-12-01T00:00:00Z --format T20

  # All models, all formats from config (CSV)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Auto-tune ML models (batting, bowling, fielding)")
    parser.add_argument("--model", choices=["batting", "bowling", "fielding", "all"], default="batting")
    ...
    models = ["batting", "bowling", "fielding"] if args.model == "all" else [args.model]
